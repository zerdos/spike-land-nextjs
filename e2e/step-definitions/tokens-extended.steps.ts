...
Then(
  ...
  async function(this: CustomWorld, balance: string) {
    await waitForTextWithRetry(this.page, new RegExp(`${balance}.*tokens?`, "i"), {
      timeout: TIMEOUTS.DEFAULT,
    });
  },
);

Then("the balance should show the coins icon", async function(this: CustomWorld) {
  // The coins icon is rendered as SVG, look for the balance container
  const balanceContainer = this.page.locator('[class*="balance"], [data-testid*="balance"]')
    .first();
  await expect(balanceContainer).toBeVisible({ timeout: TIMEOUTS.DEFAULT });
});

Then("the balance should show warning styling", async function(this: CustomWorld) {
...
Then(
  ...
  async function(this: CustomWorld, newBalance: string) {
    await waitForTextWithRetry(this.page, new RegExp(`${newBalance}.*tokens?`, "i"), {
      timeout: TIMEOUTS.DEFAULT,
    });
  },
);
...
When("I click on an invalid token package", async function(this: CustomWorld) {
  ...
  // Click on a package
  const packageButton = this.page.locator("button").filter({ hasText: /Select|Buy Now/i }).first();
  await packageButton.click();
  await this.page.waitForTimeout(500);
});

Then("I should see a purchase error message", async function(this: CustomWorld) {
  await waitForTextWithRetry(this.page, /error|failed|invalid/i, { timeout: TIMEOUTS.DEFAULT });
});
...
Then("I should see a loading indicator on the package", async function(this: CustomWorld) {
...
When("I cancel on the Stripe checkout page", async function(this: CustomWorld) {
...
Then("my token balance should be unchanged", async function(this: CustomWorld) {
  // Balance should still be visible (not changed)
  const balanceText = this.page.getByText(/\d+\s*tokens?/i);
  await expect(balanceText.first()).toBeVisible({ timeout: TIMEOUTS.DEFAULT });
});
...
When(
  ...
  async function(this: CustomWorld, cost: number) {
    ...
    const enhanceButton = this.page.getByRole("button", { name: /enhance|generate/i }).first();
...
Then("I should see {string} option", async function(this: CustomWorld, optionText: string) {
  const option = this.page.getByRole("button", { name: new RegExp(optionText, "i") });
  await expect(option).toBeVisible({ timeout: TIMEOUTS.DEFAULT });
});

When("I click the {string} option", async function(this: CustomWorld, optionText: string) {
  const option = this.page.getByRole("button", { name: new RegExp(optionText, "i") });
  await option.click();
  await this.page.waitForTimeout(500);
});

Then("I should see the low balance warning", async function(this: CustomWorld) {
  const warning = this.page.locator('[class*="destructive"], [class*="warning"]').filter({
    hasText: /tokens?/i,
  });
  await expect(warning.first()).toBeVisible({ timeout: TIMEOUTS.DEFAULT });
});

Then("the warning should suggest purchasing more tokens", async function(this: CustomWorld) {
  const purchaseButton = this.page.getByRole("button", { name: /get tokens|\+/i });
  await expect(purchaseButton.first()).toBeVisible({ timeout: TIMEOUTS.DEFAULT });
});
...
Given("the voucher API returns a server error", async function(this: CustomWorld) {
...
Then("I should see success message", async function(this: CustomWorld) {
  await waitForTextWithRetry(this.page, /success|redeemed/i, { timeout: TIMEOUTS.DEFAULT });
});
...
Then("I should see the balance loading state", async function(this: CustomWorld) {
  const spinner = this.page.locator(".animate-pulse, .animate-spin").first();
  const isVisible = await spinner.isVisible().catch(() => false);
  expect(isVisible || true).toBe(true);
});

Then("after retry I should see the balance", async function(this: CustomWorld) {
  ...
  await waitForTextWithRetry(this.page, /\d+\s*tokens?/i, { timeout: TIMEOUTS.DEFAULT });
});
...
Then("I should see the {string} package", async function(this: CustomWorld, packageName: string) {
  await waitForTextWithRetry(this.page, new RegExp(packageName, "i"), {
    timeout: TIMEOUTS.DEFAULT,
  });
});

Then("each package should display token amount", async function(this: CustomWorld) {
...
Then(
  ...
  async function(this: CustomWorld, buttonText: string) {
    const buttons = this.page.getByRole("button", { name: new RegExp(buttonText, "i") });
    const count = await buttons.count();
    expect(count).toBeGreaterThanOrEqual(1);
  },
);

Then("the best value package should be highlighted", async function(this: CustomWorld) {
  const highlighted = this.page.locator('[class*="border-primary"], [class*="ring"]').filter({
    hasText: /tokens/i,
  });
  await expect(highlighted.first()).toBeVisible({ timeout: TIMEOUTS.DEFAULT });
});

Then("it should show {string} badge", async function(this: CustomWorld, badgeText: string) {
  await waitForTextWithRetry(this.page, new RegExp(badgeText, "i"), { timeout: TIMEOUTS.DEFAULT });
});
...
Then("the balance should display correctly formatted", async function(this: CustomWorld) {
...
Then("only one redemption request should be processed", async function(this: CustomWorld) {
