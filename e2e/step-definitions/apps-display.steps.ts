...
Given('I am on the client page without displayId parameter', async function (this: CustomWorld) {
  await this.page.goto(`${this.baseUrl}/client`);
  await this.page.waitForLoadState('networkidle');
});

Given('I am on the client page with displayId parameter', async function (this: CustomWorld) {
...
Then('I should see the page heading {string}', async function (this: CustomWorld, heading: string) {
...
Then('I should see the app card {string}', async function (this: CustomWorld, appTitle: string) {
  const card = this.page.getByRole('heading', { name: appTitle });
  await expect(card).toBeVisible({ timeout: 10000 });
});

Then('I should see the app description {string}', async function (this: CustomWorld, description: string) {
  const element = this.page.getByText(description);
  await expect(element).toBeVisible({ timeout: 10000 });
});

Then('I should see the following tags:', async function (this: CustomWorld, dataTable: DataTable) {
  const tags = dataTable.rows().map(row => row[0]).filter((tag): tag is string => tag !== undefined);
...
When('I click the {string} button for {string}', async function (this: CustomWorld, buttonText: string, appName: string) {
  ...
  const appCard = this.page.locator('div', { has: this.page.getByRole('heading', { name: appName }) });
...
Then('I should see the display status indicator', async function (this: CustomWorld) {
  // Look for the status indicator div
  const statusIndicator = this.page.locator('.absolute.top-4.left-4');
  await expect(statusIndicator).toBeVisible({ timeout: 10000 });
});

Then('the status indicator should show {string}', async function (this: CustomWorld, statusText: string) {
  const statusElement = this.page.getByText(statusText, { exact: false });
  await expect(statusElement).toBeVisible({ timeout: 10000 });
});

Then('the status indicator should contain a display ID', async function (this: CustomWorld) {
  // Look for the ID pattern in the status indicator
  const idPattern = /ID: [a-zA-Z0-9-]+/;
  const statusText = await this.page.locator('.absolute.top-4.left-4').textContent();
...
Then('I should see the QR code for connecting clients', async function (this: CustomWorld) {
  const qrCode = this.page.locator('img[alt="QR Code"]');
  await expect(qrCode).toBeVisible({ timeout: 10000 });
});
...
Then('I should see the connection status indicator', async function (this: CustomWorld) {
  // Look for connection status (either "Connected" or "Connecting...")
  const statusIndicator = this.page.locator('div').filter({ hasText: /Connected|Connecting/ }).first();
