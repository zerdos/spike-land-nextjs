...
Then("I should see the API Keys section title", async function(this: CustomWorld) {
  const title = this.page.getByText(/API Keys/i).first();
  await expect(title).toBeVisible({ timeout: 10000 });
});

Then("I should see REST API usage instructions", async function(this: CustomWorld) {
  const restApi = this.page.getByText(/REST API/i);
  await expect(restApi.first()).toBeVisible({ timeout: 10000 });
});

Then("I should see MCP Server usage instructions", async function(this: CustomWorld) {
  const mcpServer = this.page.getByText(/MCP Server/i);
  await expect(mcpServer.first()).toBeVisible({ timeout: 10000 });
});

Then("I should see a link to test API keys", async function(this: CustomWorld) {
  const testLink = this.page.getByRole("link", { name: /test your api keys/i });
...
Then("I should see the {string} input field", async function(this: CustomWorld, fieldName: string) {
  const input = this.page.getByRole("textbox", { name: new RegExp(fieldName, "i") });
...
Then(
  ...
  async function(this: CustomWorld, buttonName: string) {
    const dialog = this.page.getByRole("dialog");
    const button = dialog.getByRole("button", { name: new RegExp(buttonName, "i") });
    await expect(button).toBeVisible({ timeout: 10000 });
  },
);
...
Then("I should see the API key created dialog", async function(this: CustomWorld) {
  const dialog = this.page.getByRole("dialog");
  await expect(dialog).toBeVisible({ timeout: 10000 });
...
Then(
  ...
  async function(this: CustomWorld, prefix: string) {
    const keyCode = this.page.locator("code").filter({ hasText: new RegExp(`^${prefix}`) });
    await expect(keyCode).toBeVisible({ timeout: 10000 });
  },
);

Then("I should see a warning to copy the key now", async function(this: CustomWorld) {
  const warning = this.page.getByText(/copy.*key.*now|will not be.*shown again/i);
...
Then("I should see the masked API key format", async function(this: CustomWorld) {
  const maskedKey = this.page.locator("code").filter({ hasText: /sk_live.*\*\*\*\*/ });
...
When(
  ...
  async function(this: CustomWorld, keyName: string) {
    const keyRow = this.page.locator("div, tr").filter({ hasText: keyName }).first();
...
Then("I should be able to cancel the deletion", async function(this: CustomWorld) {
...
Then("I should see instructions to create an API key", async function(this: CustomWorld) {
