...
function _createMockFiles(count: number, options?: { oversized?: boolean; nonImage?: boolean }) {
  const files = [];
...
Given('I mock batch upload with varying speeds', async function (this: CustomWorld) {
  let uploadCount = 0;
...
Given('I have uploaded {int} images in a batch', async function (this: CustomWorld, imageCount: number) {
...
Given('I have selected {int} images for enhancement', async function (this: CustomWorld, count: number) {
...
Given('I have {int} images selected for enhancement', async function (this: CustomWorld, count: number) {
  const checkboxes = this.page.locator('input[type="checkbox"]');
  for (let i = 0; i < count; i++) {
    await checkboxes.nth(i).check();
  }
...
Given('I have {int} images selected', async function (this: CustomWorld, count: number) {
  const checkboxes = this.page.locator('input[type="checkbox"]');
  for (let i = 0; i < count; i++) {
    await checkboxes.nth(i).check();
  }
...
Given('I have images selected for batch enhancement', async function (this: CustomWorld) {
  const checkboxes = this.page.locator('input[type="checkbox"]');
...
Given('the tier selection modal is open', async function (this: CustomWorld) {
  const enhanceButton = this.page.getByRole('button', { name: /Enhance Selected/i });
  await enhanceButton.click();
  await this.page.waitForTimeout(300);
});

Given('I have started batch enhancement for {int} images', async function (this: CustomWorld, count: number) {
...
Given('I have {int} images selected for deletion', async function (this: CustomWorld, count: number) {
  const checkboxes = this.page.locator('input[type="checkbox"]');
  for (let i = 0; i < count; i++) {
    await checkboxes.nth(i).check();
  }
...
Given('I had a batch enhancement that partially failed', async function (this: CustomWorld) {
...
Given('I have completed several batch enhancements', async function (this: CustomWorld) {
...
// When steps
When('I select multiple valid image files for upload', async function (this: CustomWorld) {
...
When('I select {int} images for batch upload', async function (this: CustomWorld, count: number) {
  const filePaths = Array.from({ length: count }, (_, i) =>
...
When('I upload {int} images in batch', async function (this: CustomWorld, count: number) {
  const filePaths = Array.from({ length: count }, (_, i) =>
    path.join(__dirname, `../fixtures/test-image-${i + 1}.jpg`)
  );
  const fileInput = this.page.locator('input[type="file"]');
...
When('I cancel one image from the batch', async function (this: CustomWorld) {
  const cancelButton = this.page.getByRole('button', { name: /Cancel|Remove/i }).first();
  await cancelButton.click();
});

When('I select images where some exceed 50MB', async function (this: CustomWorld) {
  // This would require actual large files or mocking
  // Placeholder for the step
  await this.page.waitForTimeout(100);
});

When('I select mixed files including non-images', async function (this: CustomWorld) {
  // Placeholder - would need actual files
  await this.page.waitForTimeout(100);
});

When('I select {int} images using checkboxes', async function (this: CustomWorld, count: number) {
  const checkboxes = this.page.locator('input[type="checkbox"]');
  for (let i = 0; i < count; i++) {
    await checkboxes.nth(i).check();
  }
...
When('I select {string} tier for batch enhancement', async function (this: CustomWorld, tier: string) {
...
When('I confirm the batch enhancement', async function (this: CustomWorld) {
  const confirmButton = this.page.getByRole('button', { name: /Confirm|Enhance/i });
  await confirmButton.click();
});

When('I try to batch enhance with {string}', async function (this: CustomWorld, tier: string) {
  const enhanceButton = this.page.getByRole('button', { name: /Enhance Selected/i });
  await enhanceButton.click();
  const tierOption = this.page.getByText(tier);
  await tierOption.click();
...
When('I select more than {int} images for upload', async function (this: CustomWorld, _limit: number) {
...
// Then steps
Then('I should see multiple images queued for upload', async function (this: CustomWorld) {
  const queueItems = this.page.locator('[data-testid="upload-queue-item"]');
  const count = await queueItems.count();
  expect(count).toBeGreaterThan(1);
});

Then('each image should show an upload progress indicator', async function (this: CustomWorld) {
  const progressBars = this.page.locator('[role="progressbar"]');
  const count = await progressBars.count();
  expect(count).toBeGreaterThan(0);
});

Then('I should see all {int} image file names listed', async function (this: CustomWorld, count: number) {
...
Then('each file should have a status indicator', async function (this: CustomWorld) {
  const statusElements = this.page.locator('[data-testid*="status"]');
  const count = await statusElements.count();
  expect(count).toBeGreaterThan(0);
});

Then('each image should show individual upload progress', async function (this: CustomWorld) {
  const progressBars = this.page.locator('[role="progressbar"]');
  const count = await progressBars.count();
  expect(count).toBeGreaterThan(0);
});

Then('faster uploads should complete before slower ones', async function (this: CustomWorld) {
...
Then('that image should be removed from the queue', async function (this: CustomWorld) {
...
Then('other images should continue uploading', async function (this: CustomWorld) {
  const progressBars = this.page.locator('[role="progressbar"]');
  const count = await progressBars.count();
  expect(count).toBeGreaterThanOrEqual(0);
});

Then('successful uploads should show success status', async function (this: CustomWorld) {
  const successIndicators = this.page.locator('[data-testid*="success"]');
  const count = await successIndicators.count();
  expect(count).toBeGreaterThan(0);
});

Then('failed uploads should show error status', async function (this: CustomWorld) {
...
Then('valid images should proceed to upload', async function (this: CustomWorld) {
  const uploadQueue = this.page.locator('[data-testid="upload-queue"]');
  await expect(uploadQueue).toBeVisible();
});

Then('I should see size validation errors for rejected files', async function (this: CustomWorld) {
  const error = this.page.getByText(/size.*limit/i);
  await expect(error).toBeVisible();
});

Then('only valid image files should be queued', async function (this: CustomWorld) {
  // Verify only image files in queue
  await this.page.waitForTimeout(300);
});

Then('non-image files should show validation errors', async function (this: CustomWorld) {
  const error = this.page.getByText(/invalid.*file.*type/i);
  await expect(error).toBeVisible();
});

Then('I should see all {int} images in the images list', async function (this: CustomWorld, count: number) {
  const images = this.page.locator('[data-testid="image-item"]');
  const actualCount = await images.count();
  expect(actualCount).toBe(count);
});

Then('images should be sorted by upload date', async function (this: CustomWorld) {
  // This would require checking timestamps
  await this.page.waitForTimeout(100);
});

Then('I should see {string} indicator', async function (this: CustomWorld, text: string) {
...
Then('each tier should show total token cost for {int} images', async function (this: CustomWorld, _imageCount: number) {
  const costIndicators = this.page.locator('[data-testid*="tier-cost"]');
  const count = await costIndicators.count();
  expect(count).toBeGreaterThan(0);
});

Then('all {int} images should start processing', async function (this: CustomWorld, _count: number) {
...
Then('my token balance should decrease by the total cost', async function (this: CustomWorld) {
  // This would require checking the token balance before/after
  await this.page.waitForTimeout(300);
});

Then('I should see insufficient tokens warning', async function (this: CustomWorld) {
...
Then('I should see individual progress for each image', async function (this: CustomWorld) {
  const progressBars = this.page.locator('[role="progressbar"]');
  const count = await progressBars.count();
  expect(count).toBeGreaterThan(0);
});
...
Then('completed enhancements should show before pending ones', async function (this: CustomWorld) {
...
Then('all {int} selected images should be removed', async function (this: CustomWorld, _count: number) {
...
Then('all {int} images should remain in the list', async function (this: CustomWorld, count: number) {
  const images = this.page.locator('[data-testid="image-item"]');
  const actualCount = await images.count();
  expect(actualCount).toBeGreaterThanOrEqual(count);
});

Then('I should see a warning about the limit', async function (this: CustomWorld) {
  const warning = this.page.getByText(/maximum.*20.*images/i);
  await expect(warning).toBeVisible();
});

Then('only the first {int} images should be queued', async function (this: CustomWorld, limit: number) {
  const queueItems = this.page.locator('[data-testid="upload-queue-item"]');
  const count = await queueItems.count();
...
Then('I should see batch enhancement completion times', async function (this: CustomWorld) {
...
Then('I should see which images were processed together', async function (this: CustomWorld) {
  // Verify batch grouping in UI
  await this.page.waitForTimeout(100);
});

Then('{string} should show {string}', async function (this: CustomWorld, tier: string, costText: string) {
