...
Then('I should see camera control buttons', async function (this: VideoWallWorld) {
  ...
  const toggleCameraBtn = clientContext.page.locator('[data-testid="toggle-camera"], button:has-text("Camera")');
  const toggleMicBtn = clientContext.page.locator('[data-testid="toggle-mic"], button:has-text("Microphone"), button:has-text("Mute")');
...
When('I click the {string} button', async function (this: VideoWallWorld, buttonText: string) {
  ...
  const buttonMap: Record<string, string> = {
    ...
    'Toggle Microphone': '[data-testid="toggle-mic"], button:has-text("Microphone"), button:has-text("Mute")',
...
Then('my camera should be disabled', async function (this: VideoWallWorld) {
  ...
  const disabledIndicator = clientContext.page.locator('[data-testid="camera-disabled"], [data-camera-state="off"]');
  await expect(disabledIndicator.first()).toBeVisible({ timeout: 5000 });
});

Then('I should see {string} indicator', async function (this: VideoWallWorld, indicatorText: string) {
  const clientContext = this.getClientContext('main-client');
  if (!clientContext) {
    throw new Error('Client context not found');
  }
...
Then('my microphone should be muted', async function (this: VideoWallWorld) {
  ...
  const mutedIndicator = clientContext.page.locator('[data-testid="mic-muted"], [data-mic-state="muted"]');
...
Then('my microphone should be unmuted', async function (this: VideoWallWorld) {
  ...
  const activeIndicator = clientContext.page.locator('[data-testid="mic-active"], [data-mic-state="active"]');
...
When('I set the zoom level to {float}', async function (this: VideoWallWorld, zoomLevel: number) {
  ...
  const zoomControl = clientContext.page.locator('[data-testid="zoom-control"], input[type="range"]').first();
  await zoomControl.fill(zoomLevel.toString());
  await clientContext.page.waitForTimeout(500);
});

Then('the camera zoom should be set to {float}x', async function (this: VideoWallWorld, expectedZoom: number) {
  const clientContext = this.getClientContext('main-client');
  if (!clientContext) {
    throw new Error('Client context not found');
  }
...
When('I enter {string} in the name field', async function (this: VideoWallWorld, name: string) {
  ...
  const nameInput = clientContext.page.locator('[data-testid="name-input"], input[name="name"]').first();
...
Then('the display should show {string} as my label', async function (this: VideoWallWorld, name: string) {
...
Then('the display should show my placeholder or blank feed', async function (this: VideoWallWorld) {
  const placeholder = this.displayPage.locator('[data-testid="video-placeholder"], [data-feed-disabled]');
