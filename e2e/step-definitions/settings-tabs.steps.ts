...
Then("I should see the user avatar in profile section", async function(this: CustomWorld) {
  const avatar = this.page.locator('[data-testid="profile-tab"] .h-20, [class*="avatar"]').first();
  await expect(avatar).toBeVisible({ timeout: TIMEOUTS.DEFAULT });
});

Then("the avatar should display user initials if no image", async function(this: CustomWorld) {
  // Look for avatar fallback with initials
  const fallback = this.page.locator('[class*="avatar-fallback"], span:has-text(/^[A-Z]{1,2}$/)')
    .first();
...
Then("I should see the OAuth managed message", async function(this: CustomWorld) {
...
Then("I should see the API key instructions", async function(this: CustomWorld) {
  await waitForTextWithRetry(this.page, /REST API|MCP Server/i, { timeout: TIMEOUTS.DEFAULT });
});

Then("I should see the API key created success dialog", async function(this: CustomWorld) {
  const dialog = this.page.getByRole("dialog");
  await expect(dialog).toBeVisible({ timeout: TIMEOUTS.DEFAULT });
  await waitForTextWithRetry(this.page, /API Key Created/i, { timeout: TIMEOUTS.DEFAULT });
});
...
Then("I should see the copy warning message", async function(this: CustomWorld) {
...
When("I click the copy API key button", async function(this: CustomWorld) {
  const copyButton = this.page.locator('[data-testid="copy-api-key"], button[aria-label*="copy"]')
    .first();
  await copyButton.click();
  await this.page.waitForTimeout(500);
});

Then("I should see the copy success indicator", async function(this: CustomWorld) {
  // Look for checkmark or "Copied" text
  const copied = this.page.getByText(/copied/i).or(this.page.locator('[class*="check"]')).first();
...
Then("the key should be removed from the list", async function(this: CustomWorld) {
...
When("I toggle the email notifications switch", async function(this: CustomWorld) {
...
When("I toggle the push notifications switch", async function(this: CustomWorld) {
  const toggle = this.page.locator(
...
Then("the email notifications should be toggled", async function(this: CustomWorld) {
...
Then("the push notifications should be toggled", async function(this: CustomWorld) {
  const toggle = this.page.locator(
    '[data-testid="push-notifications-switch"], #push-notifications',
  );
  await expect(toggle).toBeVisible({ timeout: TIMEOUTS.DEFAULT });
...
Then("the email notifications toggle should reflect the change", async function(this: CustomWorld) {
...
Then("I should see the public profile toggle", async function(this: CustomWorld) {
  const toggle = this.page.locator('[data-testid="public-profile-switch"], #public-profile');
  await expect(toggle).toBeVisible({ timeout: TIMEOUTS.DEFAULT });
});

Then("I should see the show activity toggle", async function(this: CustomWorld) {
  const toggle = this.page.locator('[data-testid="show-activity-switch"], #show-activity');
  await expect(toggle).toBeVisible({ timeout: TIMEOUTS.DEFAULT });
});
...
Then("the public profile setting should be toggled", async function(this: CustomWorld) {
  const toggle = this.page.locator('[data-testid="public-profile-switch"], #public-profile');
  await expect(toggle).toBeVisible({ timeout: TIMEOUTS.DEFAULT });
});

Then("the show activity setting should be toggled", async function(this: CustomWorld) {
  const toggle = this.page.locator('[data-testid="show-activity-switch"], #show-activity');
  await expect(toggle).toBeVisible({ timeout: TIMEOUTS.DEFAULT });
});
...
Then("the delete button should be styled as destructive", async function(this: CustomWorld) {
  const deleteButton = this.page.locator('[data-testid="delete-account-button"]');
  const classList = await deleteButton.getAttribute("class");
  expect(classList).toContain("destructive");
});

Then("I should see the delete account dialog", async function(this: CustomWorld) {
  const dialog = this.page.locator('[data-testid="delete-dialog"], [role="dialog"]');
  await expect(dialog).toBeVisible({ timeout: TIMEOUTS.DEFAULT });
});
...
Then(
  ...
  async function(this: CustomWorld) {
    await waitForTextWithRetry(this.page, /cannot be undone|permanently delete/i, {
      timeout: TIMEOUTS.DEFAULT,
    });
  },
);
