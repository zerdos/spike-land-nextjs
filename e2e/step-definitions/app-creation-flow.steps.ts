...
Then("I should see the subscription pricing options", async function(this: CustomWorld) {
  const monthlyOption = this.page.locator('[data-testid="subscription-monthly"]');
  const yearlyOption = this.page.locator('[data-testid="subscription-yearly"]');
...
Then(
  ...
  async function(this: CustomWorld, stepNumber: number) {
    ...
    await expect(stepTitle).toContainText(expectedSteps[stepNumber], { timeout: TIMEOUTS.DEFAULT });
  },
);
...
Then("I should see the character count warning", async function(this: CustomWorld) {
  const warning = this.page.locator('[data-testid="character-count-warning"], .character-warning');
  await expect(warning).toBeVisible({ timeout: TIMEOUTS.DEFAULT });
});

Then("characters over the limit should be truncated", async function(this: CustomWorld) {
  const wizard = getWizard(this);
  const descField = await wizard.getAppDescriptionTextarea();
...
Then("I should see the unsaved changes confirmation dialog", async function(this: CustomWorld) {
...
When("I confirm discarding changes", async function(this: CustomWorld) {
  const discardButton = this.page.getByRole("button", { name: /Discard|Yes|Confirm/i });
...
Then("I should remain on the wizard", async function(this: CustomWorld) {
  await expect(this.page).toHaveURL(/\/my-apps\/new/, { timeout: TIMEOUTS.DEFAULT });
});

// Error handling steps
Given("the server will return a submission error", async function(this: CustomWorld) {
...
Then("I should remain on the review step", async function(this: CustomWorld) {
  const wizard = getWizard(this);
  const stepTitle = await wizard.getStepTitle();
  await expect(stepTitle).toContainText("Review", { timeout: TIMEOUTS.DEFAULT });
});

// Accessibility steps
Then("I should be able to tab through form fields", async function(this: CustomWorld) {
  await this.page.keyboard.press("Tab");
  const focusedElement = this.page.locator(":focus");
  await expect(focusedElement).toBeVisible();
});

Then("the focused field should have a visible focus indicator", async function(this: CustomWorld) {
  const focusedElement = this.page.locator(":focus");
  await expect(focusedElement).toBeVisible();
...
When("I press Enter on the {string} button", async function(this: CustomWorld, buttonText: string) {
  const button = this.page.getByRole("button", { name: new RegExp(buttonText, "i") });
  await button.focus();
...
Then("all form fields should have associated labels", async function(this: CustomWorld) {
...
Then("required fields should be indicated with aria-required", async function(this: CustomWorld) {
...
// App creation helper
Given("I create an app named {string}", async function(this: CustomWorld, appName: string) {
  const wizard = getWizard(this);
