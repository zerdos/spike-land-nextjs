...
async function mockApiTimeout(world: CustomWorld, pattern: string, delay: number = 30000) {
  await world.page.route(pattern, async (route) => {
...
Given("the API returns a 401 unauthorized error", async function(this: CustomWorld) {
...
Then("I should see the option to sign in again", async function(this: CustomWorld) {
...
// Rate Limiting
Given("the API returns a 429 rate limit error", async function(this: CustomWorld) {
  await mockApiError(
    this,
    "**/api/**",
    429,
...
Then("I should see the rate limit error", async function(this: CustomWorld) {
  await waitForTextWithRetry(this.page, /rate limit/i, { timeout: TIMEOUTS.DEFAULT });
});

Then("I should see the retry after time", async function(this: CustomWorld) {
  await waitForTextWithRetry(this.page, /retry|wait|seconds/i, { timeout: TIMEOUTS.DEFAULT });
});

Then("I should see {string} information", async function(this: CustomWorld, text: string) {
  await waitForTextWithRetry(this.page, new RegExp(text, "i"), { timeout: TIMEOUTS.DEFAULT });
});
...
When("I submit a form with invalid data", async function(this: CustomWorld) {
  ...
  const createButton = this.page.getByRole("button", { name: /Create API Key/i });
...
Then("I should see a validation error message", async function(this: CustomWorld) {
  ...
  const createKeyButton = this.page.getByRole("button", { name: /^Create Key$/i });
  await expect(createKeyButton).toBeDisabled({ timeout: TIMEOUTS.DEFAULT });
});

Then("the error should indicate which field is invalid", async function(this: CustomWorld) {
...
When("I try to create an API key without a name", async function(this: CustomWorld) {
  const createButton = this.page.getByRole("button", { name: /Create API Key/i });
  await createButton.click();
  await this.page.waitForTimeout(300);
});

Then("the create button should be disabled", async function(this: CustomWorld) {
  const createKeyButton = this.page.getByRole("button", { name: /^Create Key$/i });
  await expect(createKeyButton).toBeDisabled({ timeout: TIMEOUTS.DEFAULT });
});
...
When("I enter a name longer than 50 characters", async function(this: CustomWorld) {
  const input = this.page.getByRole("textbox", { name: /key name/i });
...
Then("I should see a validation error for name length", async function(this: CustomWorld) {
...
Given("the API returns a 503 service unavailable error", async function(this: CustomWorld) {
...
Then("I should see a service unavailable message", async function(this: CustomWorld) {
  await waitForTextWithRetry(
    this.page,
...
Then("I should see a timeout error message", async function(this: CustomWorld) {
  await waitForTextWithRetry(
    this.page,
...
Then("I should see an offline error message", async function(this: CustomWorld) {
  await waitForTextWithRetry(
    this.page,
...
Then("I should see instructions to check connection", async function(this: CustomWorld) {
  await waitForTextWithRetry(
    this.page,
...
When("I attempt to generate an image via MCP", async function(this: CustomWorld) {
...
Then("I should see {string} error", async function(this: CustomWorld, errorText: string) {
  await waitForTextWithRetry(this.page, new RegExp(errorText, "i"), { timeout: TIMEOUTS.DEFAULT });
});

Then("I should see the option to purchase tokens", async function(this: CustomWorld) {
  const purchaseOption = this.page.getByRole("button", { name: /purchase|buy|get tokens/i });
  await expect(purchaseOption).toBeVisible({ timeout: TIMEOUTS.DEFAULT });
});

When("I attempt to generate an image with empty prompt", async function(this: CustomWorld) {
...
Then("I should see a validation error for prompt", async function(this: CustomWorld) {
  await waitForTextWithRetry(this.page, /prompt.*required|invalid prompt/i, {
    timeout: TIMEOUTS.DEFAULT,
  });
});

When("I attempt to modify an image with invalid URL", async function(this: CustomWorld) {
...
When("I check status for a non-existent job", async function(this: CustomWorld) {
...
Given("I have {int} API keys already", async function(this: CustomWorld, count: number) {
...
Then("I should see {string} message", async function(this: CustomWorld, message: string) {
  await waitForTextWithRetry(this.page, new RegExp(message, "i"), { timeout: TIMEOUTS.DEFAULT });
});

Then("I should see instructions to revoke an existing key", async function(this: CustomWorld) {
  await waitForTextWithRetry(this.page, /revoke.*existing/i, { timeout: TIMEOUTS.DEFAULT });
});
...
// Error Toast
Given("the API returns a non-critical error", async function(this: CustomWorld) {
...
Then("the toast should auto-dismiss after timeout", async function(this: CustomWorld) {
...
// Form Error States
Then("the create key button should be disabled initially", async function(this: CustomWorld) {
  const createKeyButton = this.page.getByRole("button", { name: /^Create Key$/i });
  await expect(createKeyButton).toBeDisabled({ timeout: TIMEOUTS.DEFAULT });
});
...
Then("the create key button should be enabled", async function(this: CustomWorld) {
  const createKeyButton = this.page.getByRole("button", { name: /^Create Key$/i });
