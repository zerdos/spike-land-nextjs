...
Given("I have an album with a broken image URL", async function(this: CustomWorld) {
...
Then("the canvas container should be visible", async function(this: CustomWorld) {
  const canvasPage = getCanvasPage(this);
  await canvasPage.verifyCanvasVisible();
});

Then("the canvas should have fullscreen styling", async function(this: CustomWorld) {
  ...
  expect(styles.position === "fixed" || styles.position === "absolute" || styles.width !== "auto")
    .toBeTruthy();
});

Then("I should see the album title in the header area", async function(this: CustomWorld) {
  const canvasPage = getCanvasPage(this);
  const title = await canvasPage.getAlbumTitle();
  await expect(title).toBeVisible({ timeout: TIMEOUTS.DEFAULT });
});

Then("the title should be visible on hover", async function(this: CustomWorld) {
  const canvasPage = getCanvasPage(this);
...
Then("the canvas toolbar should be visible", async function(this: CustomWorld) {
  const canvasPage = getCanvasPage(this);
...
Then("I should see the exit fullscreen button", async function(this: CustomWorld) {
  const canvasPage = getCanvasPage(this);
...
Then("the canvas should enter fullscreen mode", async function(this: CustomWorld) {
  // Fullscreen API check
  const isFullscreen = await this.page.evaluate(() => {
    return document.fullscreenElement !== null;
  });
  // Note: Fullscreen might be blocked in test environment
...
Then("the canvas should exit fullscreen mode", async function(this: CustomWorld) {
  const canvasPage = getCanvasPage(this);
...
When("I click the zoom in button multiple times", async function(this: CustomWorld) {
  const canvasPage = getCanvasPage(this);
  await canvasPage.clickZoomIn();
  await canvasPage.clickZoomIn();
  await canvasPage.clickZoomIn();
...
Then("the image should return to original size", async function(this: CustomWorld) {
  const canvasPage = getCanvasPage(this);
  await canvasPage.verifyImageAtOriginalSize();
});

Then("I should see the zoom level indicator", async function(this: CustomWorld) {
  const canvasPage = getCanvasPage(this);
  const indicator = await canvasPage.getZoomLevelIndicator();
  await expect(indicator).toBeVisible({ timeout: TIMEOUTS.DEFAULT });
});

Then("the zoom level should be {string}", async function(this: CustomWorld, level: string) {
  const canvasPage = getCanvasPage(this);
...
Then("the play button should change to pause", async function(this: CustomWorld) {
  const canvasPage = getCanvasPage(this);
  const pauseButton = await canvasPage.getPauseButton();
  await expect(pauseButton).toBeVisible({ timeout: TIMEOUTS.DEFAULT });
});

Then("a different image should be displayed", async function(this: CustomWorld) {
  const canvasPage = getCanvasPage(this);
  const currentSrc = await canvasPage.getDisplayedImageSrc();
  expect(currentSrc).not.toBe(this.initialImageSrc);
});
...
Then(
  ...
  async function(this: CustomWorld, opt1: string, opt2: string, opt3: string, opt4: string) {
    ...
    for (const opt of [opt1, opt2, opt3, opt4]) {
      ...
      await expect(option.first()).toBeVisible({ timeout: TIMEOUTS.DEFAULT }).catch(() => {
...
When("I select {string} second interval", async function(this: CustomWorld, seconds: string) {
  const canvasPage = getCanvasPage(this);
...
Then(
  ...
  async function(this: CustomWorld, interval: string) {
    const canvasPage = getCanvasPage(this);
    const selector = await canvasPage.getIntervalSelector();
    await expect(selector).toContainText(interval, { timeout: TIMEOUTS.DEFAULT });
  },
);
...
When("I click the rotate counter-clockwise button", async function(this: CustomWorld) {
  const canvasPage = getCanvasPage(this);
...
// Loading states
Then("I should see the loading indicator briefly", async function(this: CustomWorld) {
  const canvasPage = getCanvasPage(this);
...
Then("the loading indicator should disappear when image loads", async function(this: CustomWorld) {
  const canvasPage = getCanvasPage(this);
  const indicator = await canvasPage.getLoadingIndicator();
  await expect(indicator).not.toBeVisible({ timeout: TIMEOUTS.DEFAULT });
});

Then("I should see the loading indicator during transition", async function(this: CustomWorld) {
...
// Error handling
Then("I should see the image error placeholder", async function(this: CustomWorld) {
  const canvasPage = getCanvasPage(this);
...
Then("navigation buttons should have aria-labels", async function(this: CustomWorld) {
  const buttons = this.page.locator("button[aria-label]");
  const count = await buttons.count();
  expect(count).toBeGreaterThan(0);
});

Then("the current image should be announced", async function(this: CustomWorld) {
...
Then("focus should move through controls in logical order", async function(this: CustomWorld) {
  const focusedElement = this.page.locator(":focus");
  await expect(focusedElement).toBeVisible();
});

Then("focused elements should have visible focus indicators", async function(this: CustomWorld) {
  const focusedElement = this.page.locator(":focus");
...
Then("the slideshow order should be set to random", async function(this: CustomWorld) {
...
Then("I should see the shareable canvas URL", async function(this: CustomWorld) {
  const canvasPage = getCanvasPage(this);
...
Then("I should see {string} feedback text", async function(this: CustomWorld, text: string) {
