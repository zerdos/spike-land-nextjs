...
Then(
  "a unique connection ID should be generated",
...
Then(
  "the connection ID should be displayed",
  async function(this: VideoWallWorld) {
    const idDisplay = this.displayPage.locator(
      '[data-testid="connection-id"], [data-display-id]',
    );
...
Then(
  "the connection ID should be encoded in the QR code",
  async function(this: VideoWallWorld) {
...
When(
  "I scan the QR code with a client device",
  async function(this: VideoWallWorld) {
...
Then(
  "the client page should open with the connection ID",
  async function(this: VideoWallWorld) {
...
Then(
  "the client should automatically connect to the display",
  async function(this: VideoWallWorld) {
    const clientContext = this.getClientContext("scanned-client");
    if (!clientContext) {
      throw new Error("Client context not found");
...
Then(
  "the display should show the new client's video feed",
  async function(this: VideoWallWorld) {
...
// Manual connection
Given(
  "I am on the display page with connection ID {string}",
  async function(this: VideoWallWorld, connectionId: string) {
...
When(
  "I navigate to the client page with ID {string}",
  async function(this: VideoWallWorld, connectionId: string) {
    const clientContext = await this.createClientContext(
      "manual-client",
      "Manual User",
...
Then(
  "I should be connected to the display",
  async function(this: VideoWallWorld) {
    const clientContext = this.getClientContext("manual-client") ||
      this.getClientContext("scanned-client");
    if (!clientContext) {
      throw new Error("Client context not found");
    }
...
Then(
  "the display should show my video feed",
  async function(this: VideoWallWorld) {
    await this.displayPage.waitForTimeout(1000);
    const videoFeeds = this.displayPage.locator(
      '[data-testid="video-feed"], video',
...
Then(
  "the display should establish a WebRTC peer connection",
  async function(this: VideoWallWorld) {
    // Verify PeerJS connection was established
    const hasPeerConnection = await this.displayPage.evaluate(() => {
      return typeof (window as unknown as { Peer?: unknown; }).Peer !==
        "undefined";
    });
...
Then(
  "the connection should use STUN\\/TURN servers",
  async function(this: VideoWallWorld) {
...
Then(
  "the connection state should be {string}",
  async function(this: VideoWallWorld, state: string) {
    ...
    const stateDisplay = clientContext.page.locator(
      `[data-connection-state="${state.toLowerCase()}"]`,
    );
...
// Connection timeout
When(
  "a client attempts to connect but network is slow",
  async function(this: VideoWallWorld) {
    const clientContext = await this.createClientContext(
      "slow-client",
      "Slow User",
...
When(
  "the connection takes longer than {int} seconds",
  async function(this: VideoWallWorld, seconds: number) {
...
Then(
  "the client should see {string} error",
...
Then(
  "the client should be able to retry",
  async function(this: VideoWallWorld) {
    const clientContext = this.getClientContext("slow-client");
    if (!clientContext) {
      throw new Error("Client context not found");
    ...
    const retryButton = clientContext.page.locator(
...
When(
  "{int} clients attempt to connect simultaneously",
...
Then(
  "all {int} clients should connect successfully",
  async function(this: VideoWallWorld, count: number) {
...
Then(
  "the display should show {int} video feeds",
  async function(this: VideoWallWorld, count: number) {
    const videoFeeds = this.displayPage.locator(
      '[data-testid="video-feed"], video',
    );
...
Then(
  "each client should have a stable connection",
  async function(this: VideoWallWorld) {
...
Given(
  "I am connected to the display as a client",
  async function(this: VideoWallWorld) {
    await this.mockPeerJS(this.displayPage);
...
When(
  "the network connection is interrupted briefly",
  async function(this: VideoWallWorld) {
...
Then(
  "my video feed should resume on the display",
  async function(this: VideoWallWorld) {
    await this.displayPage.waitForTimeout(1000);
    const videoFeeds = this.displayPage.locator(
      '[data-testid="video-feed"], video',
...
Then(
  "I should reconnect to the same display",
  async function(this: VideoWallWorld) {
    const clientContext = this.getClientContext("reconnect-client");
    if (!clientContext) {
      throw new Error("Client context not found");
...
Then(
  "my video feed should appear on the display",
  async function(this: VideoWallWorld) {
    await this.displayPage.waitForTimeout(1000);
    const videoFeeds = this.displayPage.locator(
      '[data-testid="video-feed"], video',
...
When(
  "the network connection is lost for more than {int} seconds",
  async function(this: VideoWallWorld, seconds: number) {
    const clientContext = this.getClientContext("reconnect-client");
    if (!clientContext) {
      throw new Error("Client context not found");
...
Then(
  "the display should show {string} notification",
  async function(this: VideoWallWorld, notification: string) {
...
Then(
  "the layout should adjust for remaining clients",
  async function(this: VideoWallWorld) {
...
Then(
  "I should see connection status for each client",
  async function(this: VideoWallWorld) {
    const statusIndicators = this.displayPage.locator(
      '[data-testid="connection-status"], [data-status]',
    );
...
Then(
  "the status should show {string} in green",
  async function(this: VideoWallWorld, statusText: string) {
    const statusElement = this.displayPage.locator(
      `[data-testid="connection-status"]:has-text("${statusText}")`,
    );
    await expect(statusElement.first()).toBeVisible({ timeout: 5000 });

    // Verify green color (could check CSS class or computed style)
    const hasGreenStatus = await statusElement.first().evaluate(
      (el: HTMLElement) => {
...
When(
  "1 client's connection becomes unstable",
  async function(this: VideoWallWorld) {
    // Simulate unstable connection
    await this.displayPage.evaluate(() => {
      window.dispatchEvent(
        new CustomEvent("connection-quality-changed", {
...
Then(
  "the status should show {string} in yellow",
  async function(this: VideoWallWorld, statusText: string) {
    const statusElement = this.displayPage.locator(
      `[data-testid="connection-status"]:has-text("${statusText}")`,
    );
...
When("the connection quality is good", async function(this: VideoWallWorld) {
  ...
  if (clientContext) {
    await clientContext.page.evaluate(() => {
      window.dispatchEvent(
        new CustomEvent("connection-quality", { detail: { quality: "good" } }),
      );
    });
  }
});
...
Then(
  "I should see a green connection indicator",
  async function(this: VideoWallWorld) {
    const clientContext = this.getClientContext("reconnect-client") ||
      this.getAllClientContexts()[0];
    if (!clientContext) {
    ...
    const indicator = clientContext.page.locator(
      '[data-testid="quality-indicator"][data-quality="good"]',
    );
...
When("the connection quality degrades", async function(this: VideoWallWorld) {
  ...
  if (clientContext) {
    await clientContext.page.evaluate(() => {
      window.dispatchEvent(
        new CustomEvent("connection-quality", { detail: { quality: "poor" } }),
      );
    });
  }
});
...
Then(
  "I should see a yellow or red connection indicator",
  async function(this: VideoWallWorld) {
    const clientContext = this.getClientContext("reconnect-client") ||
      this.getAllClientContexts()[0];
    if (!clientContext) {
    ...
    const indicator = clientContext.page.locator(
      '[data-testid="quality-indicator"][data-quality="poor"], [data-testid="quality-indicator"][data-quality="bad"]',
    );
    await expect(indicator.first()).toBeVisible({ timeout: 5000 });
  },
);
...
Then(
  "I should see network statistics \\(latency, packet loss)",
  async function(this: VideoWallWorld) {
    const clientContext = this.getClientContext("reconnect-client") ||
      this.getAllClientContexts()[0];
    if (!clientContext) {
...
// Graceful disconnect
Given(
  "{int} clients are connected to the display",
  async function(this: VideoWallWorld, count: number) {
    await this.mockPeerJS(this.displayPage);
...
When(
  "1 client clicks the disconnect button",
  async function(this: VideoWallWorld) {
    ...
    const disconnectButton = clientContext.page.locator(
      '[data-testid="disconnect"], button:has-text("Disconnect")',
    );
...
Then(
  "the client should disconnect cleanly",
  async function(this: VideoWallWorld) {
    const clientContext = this.getClientContext("graceful-client-0");
    if (!clientContext) {
      throw new Error("Client context not found");
...
Then(
  "the display should remove the client's video feed immediately",
  async function(this: VideoWallWorld) {
    await this.displayPage.waitForTimeout(1000);
    const videoFeeds = this.displayPage.locator(
      '[data-testid="video-feed"], video',
...
Then(
  "the remaining clients should stay connected",
  async function(this: VideoWallWorld) {
...
Then(
  "the layout should adjust to {int}-column",
...
Then(
  "the connection ID should remain {string}",
  async function(this: VideoWallWorld, expectedId: string) {
    const bodyText = await this.displayPage.textContent("body");
...
Then(
  "the QR code should still show the same ID",
  async function(this: VideoWallWorld) {
    const qrCode = this.displayPage.locator(
      '[data-testid="qr-code"], canvas, svg',
    ).first();
...
Then(
  "previously connected clients should be able to reconnect",
  async function(this: VideoWallWorld) {
...
Then(
  ...
  async function(this: VideoWallWorld) {
    ...
    const homeButton = clientContext.page.locator(
      'button:has-text("Home"), a:has-text("Home")',
    );
    const retryButton = clientContext.page.locator('button:has-text("Retry")');

    const hasOption = await homeButton.first().isVisible() ||
      await retryButton.first().isVisible();
    expect(hasOption).toBeTruthy();
  },
);

Then(
  "my camera feed should stop streaming",
  async function(this: VideoWallWorld) {
    const clientContext = this.getClientContext("reconnect-client") ||
      this.getAllClientContexts()[0];
    if (!clientContext) {
