...
Then('I should see the preferences content', async function (this: CustomWorld) {
  const content = this.page.locator('[data-testid="preferences-content"], [role="tabpanel"]').filter({ hasText: /preferences|notifications/i });
...
Then('I should see the privacy content', async function (this: CustomWorld) {
  const content = this.page.locator('[data-testid="privacy-content"], [role="tabpanel"]').filter({ hasText: /privacy|visibility|delete/i });
  await expect(content.first()).toBeVisible({ timeout: 10000 });
});
...
Then('I should see the profile content', async function (this: CustomWorld) {
  const content = this.page.locator('[data-testid="profile-content"], [role="tabpanel"]').filter({ hasText: /profile|name|email/i });
  await expect(content.first()).toBeVisible({ timeout: 10000 });
});
...
Then('I should see the email notifications toggle', async function (this: CustomWorld) {
  const toggle = this.page.locator('[data-testid="email-notifications-toggle"], label:has-text("Email"), label:has-text("email")').first();
  await expect(toggle).toBeVisible({ timeout: 10000 });
});

Then('I should see the push notifications toggle', async function (this: CustomWorld) {
  const toggle = this.page.locator('[data-testid="push-notifications-toggle"], label:has-text("Push"), label:has-text("push")').first();
  await expect(toggle).toBeVisible({ timeout: 10000 });
});

Then('I should see the marketing emails toggle', async function (this: CustomWorld) {
  const toggle = this.page.locator('[data-testid="marketing-emails-toggle"], label:has-text("Marketing"), label:has-text("marketing")').first();
  await expect(toggle).toBeVisible({ timeout: 10000 });
});

// Toggle actions
When('I toggle the email notifications setting', async function (this: CustomWorld) {
  const toggle = this.page.locator('[data-testid="email-notifications-toggle"] input, input[name="email-notifications"], [role="switch"]').filter({ hasText: /email/i }).first();

  if (await toggle.count() === 0) {
    // Fallback: click the label
    const label = this.page.locator('label:has-text("Email notifications"), label:has-text("email")').first();
    await label.click();
  } else {
    await toggle.click();
  }
...
When('I toggle the push notifications setting', async function (this: CustomWorld) {
  const toggle = this.page.locator('[data-testid="push-notifications-toggle"] input, input[name="push-notifications"], [role="switch"]').filter({ hasText: /push/i }).first();

  if (await toggle.count() === 0) {
    // Fallback: click the label
    const label = this.page.locator('label:has-text("Push notifications"), label:has-text("push")').first();
    await label.click();
  } else {
    await toggle.click();
  }
...
Then('the email notifications state should change', async function (this: CustomWorld) {
  ...
  const toggle = this.page.locator('[data-testid="email-notifications-toggle"], label:has-text("Email")').first();
...
Then('the push notifications state should change', async function (this: CustomWorld) {
  // Verify some visual feedback occurred
  const toggle = this.page.locator('[data-testid="push-notifications-toggle"], label:has-text("Push")').first();
  await expect(toggle).toBeVisible();
  expect(true).toBeTruthy(); // Placeholder for actual state check
});
...
Then('I should see the {string} visibility option', async function (this: CustomWorld, option: string) {
...
Then('I should see the {string} button in the dialog', async function (this: CustomWorld, buttonText: string) {
  const dialog = this.page.locator('[role="dialog"], [role="alertdialog"]');
  const button = dialog.getByRole('button', { name: buttonText });
  await expect(button).toBeVisible({ timeout: 10000 });
});

When('I click the {string} button in the dialog', async function (this: CustomWorld, buttonText: string) {
  const dialog = this.page.locator('[role="dialog"], [role="alertdialog"]');
  const button = dialog.getByRole('button', { name: buttonText });
  await button.click();
  await this.page.waitForTimeout(500);
