...
// Page verification
Then(
  "I should see the settings page title",
...
Then("I should see the settings tabs", async function(this: CustomWorld) {
  const tabs = this.page.locator(
    '[role="tablist"], [data-testid="settings-tabs"]',
  );
...
// Content sections
Then(
  "I should see the preferences content",
  async function(this: CustomWorld) {
    const content = this.page.locator(
      '[data-testid="preferences-content"], [role="tabpanel"]',
    )
...
Then("I should see the privacy content", async function(this: CustomWorld) {
  const content = this.page.locator(
    '[data-testid="privacy-content"], [role="tabpanel"]',
  ).filter({
...
Then("I should see the profile content", async function(this: CustomWorld) {
  const content = this.page.locator(
    '[data-testid="profile-content"], [role="tabpanel"]',
  ).filter({
...
Then("I should see my avatar", async function(this: CustomWorld) {
  const avatar = this.page.locator(
    'img[alt*="avatar"], [data-testid="user-avatar"]',
  ).first();
...
Then(
  "I should see the email notifications toggle",
  async function(this: CustomWorld) {
...
Then(
  "I should see the push notifications toggle",
  async function(this: CustomWorld) {
    const toggle = this.page.locator(
...
Then(
  "I should see the marketing emails toggle",
  async function(this: CustomWorld) {
    const toggle = this.page.locator(
...
// Toggle actions
When(
  "I toggle the email notifications setting",
  async function(this: CustomWorld) {
    const toggle = this.page.locator(
...
When(
  "I toggle the push notifications setting",
  async function(this: CustomWorld) {
    const toggle = this.page.locator(
    ...
    if (await toggle.count() === 0) {
      // Fallback: click the label
      const label = this.page.locator(
        'label:has-text("Push notifications"), label:has-text("push")',
      )
        .first();
      await label.click();
    } else {
...
// State verification
Then(
  "the email notifications state should change",
  async function(this: CustomWorld) {
...
Then(
  "the push notifications state should change",
  async function(this: CustomWorld) {
    // Verify some visual feedback occurred
    const toggle = this.page.locator(
...
Then(
  "the email notifications state should be persisted",
  async function(this: CustomWorld) {
...
// Privacy settings
Then(
  "I should see the profile visibility setting",
  async function(this: CustomWorld) {
    const visibilitySetting = this.page.getByText(
      /profile visibility|visibility/i,
    );
...
// Delete account
Then(
  "the button should be styled as destructive action",
  async function(this: CustomWorld) {
...
Then(
  "I should see the delete account confirmation dialog",
  async function(this: CustomWorld) {
...
Then(
  ...
  async function(this: CustomWorld) {
    const message = this.page.getByText(
      /confirm|delete|permanent|cannot be undone/i,
    );
...
Then(
  "the delete account dialog should be closed",
  async function(this: CustomWorld) {
    const dialog = this.page.locator('[role="dialog"], [role="alertdialog"]');
