...
Given('I mock a successful enhancement with {int} token cost', async function (this: CustomWorld, cost: number) {
...
When('I return from successful Stripe checkout', async function (this: CustomWorld) {
  await this.page.goto(`${this.baseUrl}/enhance/${mockImageId}?success=true&session_id=cs_test_123`);
  await this.page.waitForLoadState('networkidle');
});
...
Then('I should see enhancement status {string}', async function (this: CustomWorld, status: string) {
  ...
  const statusElement = this.page.locator('[data-job-status]').getByText(new RegExp(status, 'i')).or(
...
Then('my token balance should decrease to {int} tokens', async function (this: CustomWorld, expectedBalance: number) {
...
Then('I should see enhancement status {string} in the version grid', async function (this: CustomWorld, status: string) {
