...
**File**: `src/lib/ai/audience-analysis.ts` (NEW)
...
**File**: `src/app/api/audience/[briefId]/analyze/route.ts` (NEW)

**Endpoint**: `POST /api/audience/[briefId]/analyze`

**Authentication**: Required (NextAuth session)

**Request**: No body required (briefId from URL)
...
1. **Migration**: `prisma/migrations/YYYYMMDD_add_audience_analysis/migration.sql`
   - Add `AudienceAnalysis` model
   - Add relation to `CampaignBrief`

2. **Service**: `src/lib/ai/audience-analysis.ts`
   - Core analysis logic
   - Prompt engineering
   - Response parsing
   - Error handling

3. **Service Tests**: `src/lib/ai/audience-analysis.test.ts`
   - Unit tests for analysis service
   - Mock Gemini API responses
   - Test error scenarios
   - 100% coverage required

4. **API Route**: `src/app/api/audience/[briefId]/analyze/route.ts`
   - POST endpoint handler
   - Authentication & authorization
   - Request/response formatting

5. **API Tests**: `src/app/api/audience/[briefId]/analyze/route.test.ts`
   - Integration tests for API endpoint
   - Auth tests
   - Error handling tests

6. **Validation**: `packages/shared/src/validations/audience-analysis.ts`
   - Zod schemas
   - Type exports

7. **Validation Tests**: `packages/shared/src/validations/audience-analysis.test.ts`
