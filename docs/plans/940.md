...
1. **Auth Components**:
   - `sign-in-button.tsx` - Already uses shadcn Button ✅
   - `sign-out-button.tsx` - Already uses shadcn Button ✅
...
4. **Form/Settings Pages**:
   - Settings page already uses shadcn components extensively ✅
...
### 🔴 Components with Hard-coded Tailwind Colors (Verified via Grep)

**High Priority - Must be updated to use theme variables**:

1. `src/components/calendar/BestTimeHeatmap.tsx`
2. `src/components/orbit/policy/PolicyStatsDashboard.tsx`
3. `src/components/orbit/policy/PolicyViolationCard.tsx`
4. `src/components/orbit/relay/edit-history-viewer.tsx`
5. `src/components/my-apps/MiniPreview.tsx`
6. `src/components/orbit-landing/PlatformConnections.tsx`
7. `src/components/orbit-landing/ABTestingSection.tsx`
8. `src/components/orbit-landing/AIAutomationSection.tsx`
9. `src/components/orbit-landing/EcosystemOverview.tsx`
10. `src/components/orbit-landing/OrbitHero.tsx`

**Pattern Found**: `bg-blue-*`, `bg-gray-*`, `bg-slate-*`, `bg-zinc-*`, `bg-green-*`, `bg-red-*`

**Required Change**: Replace with semantic tokens from `globals.css`:

- `bg-primary`, `bg-secondary`, `bg-muted`, `bg-accent`
- `bg-aurora-green`, `bg-aurora-teal`, `bg-aurora-lime`, `bg-aurora-yellow`
...
1. **Hard-coded Colors** (10 files - must fix for theme consistency) ⭐
2. **Auth Components** (already mostly done ✅)
3. **App Cards & Lists** (high visibility)
4. **Form Components** (consistency critical)
5. **Modal/Dialog Patterns** (accessibility critical)
6. **Navigation** (user experience critical)

**Example Migrations**:

#### 3.0 Fix Hard-coded Colors (NEW TOP PRIORITY)
...
className = "bg-blue-500 text-gray-200 border-slate-300";

// AFTER: Theme variables
className = "bg-primary text-muted-foreground border-border";

// Or for aurora-specific styling:
className = "bg-aurora-teal text-foreground border-white/10";
```

**Color Mapping Reference**:

- `bg-blue-*` → `bg-primary` or `bg-accent` (teal)
- `bg-green-*` → `bg-aurora-green`
- `bg-red-*` → `bg-destructive`
- `bg-yellow-*` → `bg-aurora-yellow`
- `bg-gray-*`, `bg-zinc-*`, `bg-slate-*` → `bg-muted`, `bg-secondary`, `bg-card`
...
#### 3.2 Custom Dialogs → Dialog Component
