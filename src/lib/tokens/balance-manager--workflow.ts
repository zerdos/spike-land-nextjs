/**
 * Workflow-safe version of balance-manager
 *
 * This file uses try-catch--no-track to avoid bundling @vercel/kv
 * which uses EventTarget (not available in workflow runtime).
 *
 * Use this file ONLY in workflow files (*.workflow.ts).
 * For regular app code, use balance-manager.ts instead.
 */
