...
const DEFAULT_SETTINGS: SmartRoutingSettings = {
    ...
    escalation: {
        ...
        levels: [
            ...
            { level: 3, name: "Director", notifyChannels: ["email", "slack", "sms"], triggerDelayMinutes: 1440 },
        ],
        autoAssign: true,
    },
    rules: [],
...
export async function getSmartRoutingSettings(workspaceId: string): Promise<SmartRoutingSettings> {
    ...
    return { ...DEFAULT_SETTINGS, ...result.data, escalation: { ...DEFAULT_SETTINGS.escalation, ...result.data.escalation } } as SmartRoutingSettings;
}
