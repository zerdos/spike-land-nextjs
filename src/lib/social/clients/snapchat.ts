...
export class SnapchatClient implements ISocialClient {
  ...
  async validateMedia(
    ...
    if (type === "IMAGE") {
      if (fileSize > maxSize) {
        errors.push(
          `Image file size (${(fileSize / 1024 / 1024).toFixed(2)}MB) exceeds maximum of 5MB`,
        );
      }
      ...
      if (fileSize > maxVideoSize) {
        errors.push(
          `Video file size (${(fileSize / 1024 / 1024).toFixed(2)}MB) exceeds maximum of 32MB`,
        );
      }
  ...
  async createPost(
    ...
    throw new Error(
      "Story creation not yet fully implemented - requires media upload and creative creation",
    );
  }
