 ...
 * Email Configuration
 *
 * White-label email branding utilities.
 * Resolves #924
 */
...
export async function getEmailSenderInfo(workspaceId: string): Promise<{
  ...
  try {
    ...
    if (config?.emailSenderName && config?.emailSenderDomain && config?.emailSenderVerified) {
      return {
...
export async function hasCustomEmailBranding(workspaceId: string): Promise<boolean> {
  ...
  return !!(
    config &&
    (config.emailSenderVerified || config.emailHeaderLogoUrl || config.emailFooterText)
  );
}
