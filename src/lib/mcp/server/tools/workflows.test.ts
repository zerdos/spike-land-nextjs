...
describe("workflows tools", () => {
  ...
  describe("workflow_create", () => {
    it("should create a workflow with steps", async () => {
      ...
      const steps = JSON.stringify([
        ...
        { name: "Wait 1 day", type: "delay", config: { days: 1 } },
      ]);
  ...
  describe("workflow_run", () => {
    it("should start a workflow run", async () => {
      ...
      mockPrisma.workflowRun.create.mockResolvedValue({ id: "run-1", status: "PENDING" });
    ...
    it("should run without input", async () => {
      ...
      mockPrisma.workflowRun.create.mockResolvedValue({ id: "run-2", status: "PENDING" });
  ...
  describe("workflow_get_status", () => {
    it("should return run status with step progress", async () => {
      mockPrisma.workflowRun.findFirst.mockResolvedValue({
        ...
        stepExecutions: [
          { stepName: "Send Email", status: "COMPLETED", startedAt: new Date("2025-06-01T10:00:00Z"), completedAt: new Date("2025-06-01T10:01:00Z") },
          { stepName: "Wait 1 day", status: "RUNNING", startedAt: new Date("2025-06-01T10:01:00Z") },
        ],
      ...
      expect(text).toContain("Current Step:** Wait 1 day");
    });
  ...
  describe("workflow_get_logs", () => {
    it("should return step execution logs", async () => {
      mockPrisma.workflowRun.findFirst.mockResolvedValue({
        ...
        stepExecutions: [
          ...
          },
          {
            ...
            completedAt: new Date("2025-06-01T10:00:06Z"),
            output: null,
            error: null,
          },
        ],
      ...
      expect(text).toContain("Log Result");
        ...
        stepExecutions: [
          {
            ...
            status: "FAILED",
            startedAt: new Date("2025-06-01T10:00:00Z"),
            completedAt: new Date("2025-06-01T10:00:03Z"),
            output: null,
    ...
    it("should handle run with no step executions", async () => {
      mockPrisma.workflowRun.findFirst.mockResolvedValue({
        id: "run-3",
        status: "PENDING",
      ...
      const text = getText(result);
      expect(text).toContain("No step executions recorded");
    });
  });
});
