...
describe('Terms of Service Page', () => {
  ...
  describe('Acceptance of Terms Section', () => {
    it('should state terms agreement', () => {
      render(<TermsPage />)
      expect(screen.getByText(/By accessing and using the Spike Land Image Enhancement App/)).toBeInTheDocument()
    })
  ...
  describe('Account Terms Section', () => {
    ...
    it('should explain account security responsibility', () => {
      ...
      expect(container.textContent?.includes('Maintaining the confidentiality of your OAuth credentials')).toBe(true)
    })
    ...
    it('should explain account termination rights', () => {
      render(<TermsPage />)
      expect(screen.getByText(/reserve the right to terminate or suspend your account/)).toBeInTheDocument()
    })

    it('should list termination reasons', () => {
      render(<TermsPage />)
      expect(screen.getByText(/Violation of these Terms/)).toBeInTheDocument()
      expect(screen.getByText(/Suspicious activity indicating fraud or misuse/)).toBeInTheDocument()
    })
  ...
  describe('Token Economy Section', () => {
    it('should explain what tokens are', () => {
      render(<TermsPage />)
      expect(screen.getByText(/Tokens are the currency used to purchase image enhancement services/)).toBeInTheDocument()
      expect(screen.getByText(/non-refundable and non-transferable digital units/)).toBeInTheDocument()
    })
    ...
    it('should explain non-refundable policy', () => {
      render(<TermsPage />)
      expect(screen.getByText(/cannot be refunded.*except in cases of failed enhancements/)).toBeInTheDocument()
    })
    ...
    it('should mention token pricing flexibility', () => {
      render(<TermsPage />)
      expect(screen.getByText(/Token prices are subject to change without notice/)).toBeInTheDocument()
    })
  ...
  describe('Service Description Section', () => {
    ...
    it('should explain 1K enhancement', () => {
      ...
      expect(screen.getByText(/Standard resolution enhancement suitable for web use/)).toBeInTheDocument()
    })

    it('should explain 2K enhancement', () => {
      render(<TermsPage />)
      expect(screen.getByText(/2K Enhancement \(2048x2048\)/)).toBeInTheDocument()
      expect(screen.getByText(/High-resolution enhancement suitable for printing/)).toBeInTheDocument()
    })
    ...
    it('should mention Google Gemini processing', () => {
      render(<TermsPage />)
      expect(screen.getByText(/Image enhancements are processed using Google Gemini API/)).toBeInTheDocument()
    ...
    it('should explain service availability limitations', () => {
      render(<TermsPage />)
      expect(screen.getByText(/do not guarantee uninterrupted or error-free service/)).toBeInTheDocument()
  ...
  describe('Acceptable Use Policy Section', () => {
    ...
    it('should mention copyright compliance', () => {
      ...
      expect(screen.getByText(/own or have all necessary rights to use images/)).toBeInTheDocument()
    })
    ...
    it('should explain copyright infringement reporting', () => {
      ...
      expect(container.textContent?.includes('Identification of the infringing content')).toBe(true)
    })
    ...
    it('should list enforcement rights', () => {
      ...
      expect(container.textContent?.includes('Immediately suspend or terminate your account')).toBe(true)
    })
  })
  ...
  describe('Intellectual Property Section', () => {
    it('should state user retains image ownership', () => {
      render(<TermsPage />)
      expect(screen.getByText(/retain all intellectual property rights and ownership/)).toBeInTheDocument()
    })
  ...
  describe('Payment Terms Section', () => {
    ...
    it('should explain failed enhancement refund', () => {
      ...
      expect(screen.getByText(/enhancement request fails due to a service error/)).toBeInTheDocument()
    })
  ...
  describe('Limitation of Liability Section', () => {
    ...
    it('should list warranty disclaimers', () => {
      render(<TermsPage />)
      expect(screen.getByText(/Image enhancements will meet your expectations/)).toBeInTheDocument()
  ...
  describe('Dispute Resolution Section', () => {
    ...
    it('should require informal resolution first', () => {
      ...
      expect(container.textContent?.includes('attempt to resolve disputes through informal negotiation')).toBe(true)
    })
    ...
    it('should explain severability', () => {
      ...
      expect(container.textContent?.includes('remaining provisions shall remain in full force')).toBe(true)
    })
  })
  ...
  describe('Changes to Terms Section', () => {
    ...
    it('should explain acceptance by continued use', () => {
      ...
      expect(screen.getByText(/continued use of the Service after changes become effective/)).toBeInTheDocument()
    })
  ...
  describe('Legal Disclaimer', () => {
    ...
    it('should state this is a template', () => {
      const { container } = render(<TermsPage />)
      expect(container.textContent?.includes('template for informational purposes only')).toBe(true)
    })
    ...
    it('should mention jurisdiction variations', () => {
      const { container } = render(<TermsPage />)
      expect(container.textContent?.includes('customize them for your specific business model')).toBe(true)
    })

    it('should warn against use without review', () => {
      const { container } = render(<TermsPage />)
      expect(container.textContent?.includes('should not be used without professional legal review')).toBe(true)
    })
  })
  ...
  describe('Token Policy Details', () => {
    ...
    it('should list failed enhancement refund conditions', () => {
      render(<TermsPage />)
      expect(screen.getByText(/tokens will be automatically credited back to your account within 24 hours/)).toBeInTheDocument()
    })
  })
})
