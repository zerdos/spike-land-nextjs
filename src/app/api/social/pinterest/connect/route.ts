/**
 * Pinterest OAuth 2.0 Connect Route
 *
 * Initiates OAuth flow to connect Pinterest accounts
 * GET /api/social/pinterest/connect?workspaceId=xxx
 */
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  // Create Pinterest client and get auth URL
  let authUrl: string;
  try {
    const client = new PinterestClient();
    authUrl = client.getAuthUrl(redirectUri, state);
  } catch (error) {
    console.error("Pinterest OAuth setup failed:", error);
    return NextResponse.json(
      {
        error: error instanceof Error
          ? error.message
          : "Failed to initialize Pinterest OAuth. Please check server configuration.",
      },
      { status: 500 },
    );
  }

  // Store nonce in secure cookies for verification in callback
