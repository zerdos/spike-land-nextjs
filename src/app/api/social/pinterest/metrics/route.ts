/**
 * Pinterest Metrics API Route
 *
 * GET /api/social/pinterest/metrics - Fetch account analytics
 */
...
/**
 * GET /api/social/pinterest/metrics
 * Fetch Pinterest account metrics and analytics
 */
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  // Fetch metrics using Pinterest client
  ...
  if (metricsError) {
    console.error("Failed to fetch Pinterest metrics:", metricsError);
    return NextResponse.json(
      { error: "Failed to fetch Pinterest metrics" },
      { status: 500 },
    );
  }
