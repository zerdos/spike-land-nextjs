...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  if (tokenError || !tokens) {
    ...
    return clearOAuthCookies(
      NextResponse.redirect(
        redirect({ error: "Failed to connect Pinterest account. Please try again." }),
      ),
    );
  }
  ...
  if (userError || !userInfo) {
    console.error("Failed to get Pinterest user info:", userError);
    return clearOAuthCookies(
      NextResponse.redirect(
  ...
  if (dbError) {
    console.error("Failed to save Pinterest account:", dbError);
    return clearOAuthCookies(
      NextResponse.redirect(
  ...
  return clearOAuthCookies(
    NextResponse.redirect(redirect({ success: "Pinterest account connected successfully!" })),
  );
}
