 ...
 *
 * Initiates OAuth flow to connect TikTok accounts
 * GET /api/social/tiktok/connect?workspaceId=xxx
 */
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  // Create TikTok client and get auth URL
  let authUrl: string;
  try {
    ...
    return NextResponse.json(
      {
        error: error instanceof Error
          ? error.message
          : "Failed to initialize TikTok OAuth. Please check server configuration.",
      },
      { status: 500 },
    );
  }
