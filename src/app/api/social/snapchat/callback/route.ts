...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  // Set access token on client for subsequent calls
  client.setAccessToken(tokens.accessToken);

  // Get account info (organization) with the new token
  const { data: accountInfo, error: accountError } = await tryCatch(
    client.getAccountInfo(),
  );

  if (accountError || !accountInfo) {
    console.error("Failed to get Snapchat account info:", accountError);
    return clearOAuthCookies(
      NextResponse.redirect(
  ...
  // Get organizations and ad accounts
  const { data: organizations } = await tryCatch(client.getOrganizations());
  const orgId = organizations?.[0]?.id || accountInfo.platformId;

  // Get ad accounts for metadata (optional)
  const { data: adAccounts } = await tryCatch(client.getAdAccounts(orgId));
    ...
    prisma.socialAccount.upsert({
      ...
      update: {
        ...
        metadata: {
          ...
          adAccountIds,
          accountType: adAccountIds.length > 0 ? "BUSINESS" : "CREATOR",
        },
        updatedAt: new Date(),
      },
      create: {
  ...
  return clearOAuthCookies(
    NextResponse.redirect(
      redirect({ connected: "snapchat", username: accountInfo.username }),
    ),
  );
}
