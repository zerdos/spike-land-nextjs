 ...
 *
 * Initiates OAuth flow to connect Snapchat accounts
 * GET /api/social/snapchat/connect?workspaceId=xxx&workspaceSlug=xxx
 */
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  // Create Snapchat client and get auth URL
  let authUrl: string;
  try {
    ...
    return NextResponse.json(
      {
        error: error instanceof Error
          ? error.message
          : "Failed to initialize Snapchat OAuth. Please check server configuration.",
      },
      { status: 500 },
    );
  }
