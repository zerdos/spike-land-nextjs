...
function createRequest(body: Record<string, unknown>, headers?: Record<string, string>) {
  const req = new Request("http://localhost/api/create/vibe-chat", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      ...headers,
    },
    body: JSON.stringify({ slug: "test-slug", ...body }),
  });
  return req as any;
}
...
describe("Vibe Chat API Route", () => {
  ...
  describe("input validation", () => {
    ...
    it("should return 400 when slug is missing", async () => {
      const req = new Request("http://localhost/api/create/vibe-chat", {
        method: "POST",
        headers: { "Content-Type": "application/json", "x-forwarded-for": `noslug-${Date.now()}` },
        body: JSON.stringify({ content: "hello", mode: "plan" }),
      });
    ...
    it("should return 400 when slug is not a string", async () => {
      const req = new Request("http://localhost/api/create/vibe-chat", {
        method: "POST",
        headers: { "Content-Type": "application/json", "x-forwarded-for": `badslug-${Date.now()}` },
        body: JSON.stringify({ slug: 123, content: "hello", mode: "plan" }),
      });
  ...
  describe("edge cases", () => {
    it("should use 'unknown' IP when no forwarding headers present", async () => {
      ...
      const req = new Request("http://localhost/api/create/vibe-chat", {
        ...
        body: JSON.stringify({ slug: "test-slug", content: "test", mode: "plan" }),
      });
