...
export async function POST(request: NextRequest) {
  ...
  if (authError) {
    console.error("Voucher redemption error:", authError);
  ...
  if (rateLimitError) {
    console.error("Voucher redemption error:", rateLimitError);
    return NextResponse.json(
      { error: "Failed to redeem voucher" },
      { status: 500 },
    );
  ...
  if (jsonError) {
    console.error("Voucher redemption error:", jsonError);
    return NextResponse.json(
      { error: "Failed to redeem voucher" },
      { status: 500 },
    );
  ...
  const { data: result, error: redeemError } = await tryCatch(
    VoucherManager.redeem(trimmedCode, session.user.id),
  );

  if (redeemError) {
    console.error("Voucher redemption error:", redeemError);
    return NextResponse.json(
      { error: "Failed to redeem voucher" },
      { status: 500 },
    );
