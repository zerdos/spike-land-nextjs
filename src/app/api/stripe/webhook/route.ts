...
async function handleCheckoutCompleted(stripe: Stripe, session: Stripe.Checkout.Session) {
  const { userId, type, tokens, packageId, planId, tokensPerMonth, maxRollover } = session.metadata || {}
  ...
  if (type === 'subscription' && planId && tokensPerMonth) {
    ...
    console.log(`[Stripe] Created subscription for user ${userId}, credited ${tokensPerMonth} tokens`)
  }
}
