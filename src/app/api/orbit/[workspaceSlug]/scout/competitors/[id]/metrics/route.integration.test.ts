...
describe('Competitor Metrics API', () => {
  ...
  beforeAll(async () => {
    ...
    await prisma.scoutCompetitorPost.createMany({
      data: [
        { competitorId: competitor.id, platformPostId: 'p1', content: 'Post 1', postedAt: new Date(), likes: 100 },
        { competitorId: competitor.id, platformPostId: 'p2', content: 'Post 2', postedAt: new Date(), likes: 200 },
      ],
    });
  });
  ...
  it('should fetch engagement metrics for a competitor', async () => {
    const req = new NextRequest(`http://localhost/api/orbit/${workspace.slug}/scout/competitors/${competitor.id}/metrics`);
