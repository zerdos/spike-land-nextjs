...
describe('GET /api/orbit/[workspaceSlug]/inbox', () => {
  ...
  beforeEach(async () => {
    ...
    await prisma.inboxItem.createMany({
      data: [
        { workspaceId: workspace.id, type: 'MENTION', platform: SocialPlatform.TWITTER, status: InboxItemStatus.UNREAD, content: 'test', platformItemId: '1', receivedAt: new Date(), senderName: 'test', accountId: '1' },
        { workspaceId: workspace.id, type: 'MENTION', platform: SocialPlatform.FACEBOOK, status: InboxItemStatus.READ, content: 'test', platformItemId: '2', receivedAt: new Date(), senderName: 'test', accountId: '1' },
      ],
    });
  });
  ...
  it('filters by platform', async () => {
    mockedAuth.mockResolvedValue({ user: { id: '1' } });
    const req = new NextRequest(`https://localhost/api/orbit/${workspace.slug}/inbox?platform=TWITTER`);
    const res = await GET(req, { params: { workspaceSlug: workspace.slug } });
    const data = await res.json();
    expect(res.status).toBe(200);
