...
describe('Cookie Policy Page', () => {
  ...
  describe('What Are Cookies Section', () => {
    it('should explain what cookies are', () => {
      render(<CookiePage />)
      expect(screen.getByText(/small text files that are stored on your device/)).toBeInTheDocument()
    })

    it('should explain how cookies work', () => {
      render(<CookiePage />)
      expect(screen.getByText(/You visit the Spike Land Image Enhancement App/)).toBeInTheDocument()
      expect(screen.getByText(/When you return to the website, your browser sends the cookie back/)).toBeInTheDocument()
    })

    it('should define session cookies', () => {
      render(<CookiePage />)
      expect(screen.getByText(/Temporary cookies that are deleted when you close your browser/)).toBeInTheDocument()
    })
    ...
    it('should define first-party cookies', () => {
      render(<CookiePage />)
      expect(screen.getByText(/Set by the website you are visiting \(Spike Land\)/)).toBeInTheDocument()
    })

    it('should define third-party cookies', () => {
      render(<CookiePage />)
      expect(screen.getByText(/Set by other websites or services embedded in our site/)).toBeInTheDocument()
    })
  })
  ...
  describe('Essential Cookies Section', () => {
    ...
    it('should describe CSRF token details', () => {
      render(<CookiePage />)
      expect(screen.getByText(/Protects against Cross-Site Request Forgery attacks/)).toBeInTheDocument()
      expect(screen.getByText(/Required for secure form submissions/)).toBeInTheDocument()
    })

    it('should describe callback URL cookie', () => {
      render(<CookiePage />)
      expect(screen.getByText(/Stores the URL to redirect to after OAuth authentication completes/)).toBeInTheDocument()
    })
  ...
  describe('Analytics Cookies Section', () => {
    ...
    it('should explain Vercel Analytics', () => {
      ...
      expect(screen.getByText(/Tracks page views, user interactions, and performance metrics/)).toBeInTheDocument()
    })

    it('should describe Vercel Analytics data collection', () => {
      render(<CookiePage />)
      expect(screen.getByText(/Page URLs, referrer, device type, browser type/)).toBeInTheDocument()
    })

    it('should state Vercel Analytics does not use cookies', () => {
      render(<CookiePage />)
      expect(screen.getByText(/Vercel Analytics does not use cookies for tracking/)).toBeInTheDocument()
    })

    it('should explain Vercel Speed Insights', () => {
      render(<CookiePage />)
      expect(screen.getByText('Vercel Speed Insights')).toBeInTheDocument()
      expect(screen.getByText(/Monitors application performance and loading speeds/)).toBeInTheDocument()
    })

    it('should describe Speed Insights data', () => {
      render(<CookiePage />)
      expect(screen.getByText(/Page load times, Core Web Vitals, performance metrics/)).toBeInTheDocument()
    })
    ...
    it('should provide Vercel privacy policy link', () => {
      const { container } = render(<CookiePage />)
      const vercelLink = container.querySelector('a[href="https://vercel.com/legal/privacy-policy"]')
  ...
  describe('Third-Party Cookies Section', () => {
    it('should explain third-party cookies purpose', () => {
      render(<CookiePage />)
      expect(screen.getByText(/external services that are embedded in our application/)).toBeInTheDocument()
    })
    ...
    it('should explain Google OAuth', () => {
      ...
      expect(bodyText?.includes('When you click "Sign in with Google," Google sets cookies')).toBe(true)
    })
    ...
    it('should provide Google Cookies Policy link', () => {
      const { container } = render(<CookiePage />)
      const googleCookiesLink = container.querySelector('a[href="https://policies.google.com/technologies/cookies"]')
    ...
    it('should explain GitHub OAuth', () => {
      ...
      expect(bodyText?.includes('When you click "Sign in with GitHub," GitHub sets cookies')).toBe(true)
    })

    it('should provide GitHub Privacy Statement link', () => {
      const { container } = render(<CookiePage />)
      const githubLink = container.querySelector('a[href="https://docs.github.com/en/site-policy/privacy-policies/github-privacy-statement"]')
    ...
    it('should explain Stripe fraud prevention', () => {
      render(<CookiePage />)
      expect(screen.getByText(/securely process your transaction and prevent fraud/)).toBeInTheDocument()
    })

    it('should clarify we do not store credit cards', () => {
      render(<CookiePage />)
      expect(screen.getByText(/Stripe does not store your credit card information on our servers/)).toBeInTheDocument()
    })
    ...
    it('should provide Stripe Cookies Policy link', () => {
      const { container } = render(<CookiePage />)
      const stripeCookiesLink = container.querySelector('a[href="https://stripe.com/cookies-policy"]')
  ...
  describe('Managing Cookies Section', () => {
    ...
    it('should provide Firefox instructions', () => {
      ...
      expect(screen.getByText(/Click Menu \(three lines\) in the top right corner/)).toBeInTheDocument()
    })
    ...
    it('should warn about disabling cookies impact', () => {
      render(<CookiePage />)
      expect(screen.getByText(/You will not be able to log in to your account/)).toBeInTheDocument()
      expect(screen.getByText(/Google and GitHub authentication will not work/)).toBeInTheDocument()
    ...
    it('should warn about clearing cookies side effects', () => {
      render(<CookiePage />)
      expect(screen.getByText(/Clearing cookies will log you out of your account/)).toBeInTheDocument()
    })
  ...
  describe('Cookie Consent Section', () => {
    it('should explain how consent is obtained', () => {
      render(<CookiePage />)
      expect(screen.getByText(/A cookie consent banner appears on your screen/)).toBeInTheDocument()
    ...
    it('should explain Customize option', () => {
      ...
      expect(screen.getByText(/Choose which specific cookie categories to accept or reject/)).toBeInTheDocument()
    })

    it('should mention GDPR Article 7 compliance', () => {
      render(<CookiePage />)
      expect(screen.getByText(/GDPR Article 7 and the ePrivacy Directive requirements/)).toBeInTheDocument()
    })
    ...
    it('should explain how to change preferences', () => {
      render(<CookiePage />)
      expect(screen.getByText(/Look for a "Cookie Settings" link at the bottom of the page/)).toBeInTheDocument()
    })
  ...
  describe('Policy Updates Section', () => {
    ...
    it('should explain notification process', () => {
      ...
      expect(screen.getByText(/post a notice on our website for at least 30 days/)).toBeInTheDocument()
  ...
  describe('Contact Section', () => {
    ...
    it('should explain how to change cookie preferences', () => {
      render(<CookiePage />)
      expect(screen.getByText(/look for the "Cookie Settings" link at the bottom of the page/)).toBeInTheDocument()
    })
  ...
  describe('Navigation and Links', () => {
    ...
    it('should link to all major sections', () => {
      ...
      expect(container.querySelector('a[href="#managing"]')).toBeInTheDocument()
  ...
  describe('Compliance Coverage', () => {
    ...
    it('should cover user rights to withdraw consent', () => {
      render(<CookiePage />)
      expect(screen.getByText(/You can change your cookie preferences at any time/)).toBeInTheDocument()
    })
  ...
  describe('User Consent Preferences', () => {
    ...
    it('should mention 12-month consent expiration', () => {
      render(<CookiePage />)
      expect(screen.getByText(/Your cookie consent preferences are stored for 12 months/)).toBeInTheDocument()
    })
  ...
  describe('Legal Information', () => {
    ...
    it('should recommend attorney consultation', () => {
      render(<CookiePage />)
      expect(screen.getByText(/qualified attorney who specializes in privacy and data protection law/)).toBeInTheDocument()
    })
