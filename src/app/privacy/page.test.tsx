...
describe('Privacy Policy Page', () => {
  ...
  describe('Data Collection Section', () => {
    ...
    it('should explain image collection', () => {
      render(<PrivacyPage />)
      expect(screen.getByText(/Original and enhanced images uploaded for processing/)).toBeInTheDocument()
    })
    ...
    it('should list what is NOT collected', () => {
      ...
      expect(screen.getByText(/No biometric or facial recognition data extracted/)).toBeInTheDocument()
  ...
  describe('Data Storage & Retention Section', () => {
    ...
    it('should explain image cleanup process', () => {
      render(<PrivacyPage />)
      expect(screen.getByText(/Images inactive for 90 days are automatically deleted/)).toBeInTheDocument()
    ...
    it('should mention transaction retention for legal reasons', () => {
      render(<PrivacyPage />)
      expect(screen.getByText(/Transaction records retained \(legal requirement\)/)).toBeInTheDocument()
    })
  })
  ...
  describe('Children\'s Privacy Section', () => {
    it('should state age restriction', () => {
      render(<PrivacyPage />)
      expect(screen.getByText(/not intended for children under 13 years of age/)).toBeInTheDocument()
    })

    it('should state children data not collected knowingly', () => {
      render(<PrivacyPage />)
      expect(screen.getByText(/do not knowingly collect personal information from children under 13/)).toBeInTheDocument()
    })

    it('should provide contact for child account deletion', () => {
      render(<PrivacyPage />)
      expect(screen.getByText(/If you believe a child under 13 has created an account/)).toBeInTheDocument()
      expect(screen.getByText(/delete the account within 30 days/)).toBeInTheDocument()
    })

    it('should require user age confirmation', () => {
      render(<PrivacyPage />)
      expect(screen.getByText(/you are at least 13 years old or have parental consent/)).toBeInTheDocument()
    })
  })
  ...
  describe('Contact Section', () => {
    ...
    it('should mention GDPR Article 30 compliance', () => {
      render(<PrivacyPage />)
      expect(screen.getByText(/maintain records of our data processing activities per GDPR Article 30/)).toBeInTheDocument()
    })

    it('should provide ICO link for UK complaints', () => {
      render(<PrivacyPage />)
      const icoLink = screen.getByRole('link', { name: /Information Commissioner's Office \(ICO\)/ })
  ...
  describe('Legal Disclaimer', () => {
    ...
    it('should mention privacy laws complexity', () => {
      render(<PrivacyPage />)
      expect(screen.getByText(/privacy laws are complex and evolve frequently/)).toBeInTheDocument()
    })
  })
