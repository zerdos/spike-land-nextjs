...
describe("ModeToggle", () => {
  ...
  it("shows sun icon and correct label in light mode", async () => {
    ...
    await waitFor(() => {
      ...
      expect(button).toHaveAttribute("aria-label", "Light mode active. Click to switch to dark mode.")
    })
  })
  ...
  it("shows moon icon and correct label in dark mode", async () => {
    ...
    await waitFor(() => {
      const button = screen.getByRole("button")
      expect(button).toBeInTheDocument()
      expect(button).toHaveAttribute("aria-label", "Dark mode active. Click to use system preference.")
    })
  })
  ...
  it("contains screen reader text", async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText("Light mode active. Click to switch to dark mode.")).toBeInTheDocument()
    })
  })
