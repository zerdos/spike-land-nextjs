...
export function InboxEscalationStatus({ status, level, slaDeadline, onEscalate, onResolve }: InboxEscalationStatusProps) {
    ...
    return (
        <div className="flex items-center gap-2">
            ...
            {slaDeadline && status !== "RESOLVED" && (
                <div className={`text-xs flex items-center gap-1 ${isBreached ? "text-red-600 font-bold" : "text-gray-500"}`}>
                    <Clock className="w-3 h-3" />
                    {isBreached ? "SLA Breached" : `Due ${new Date(slaDeadline).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}`}
                </div>
            )}
