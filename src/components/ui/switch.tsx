...
>(({ className, ...props }, ref) => (
  <SwitchPrimitives.Root
    className={cn(
      "group peer relative inline-flex h-7 w-14 shrink-0 cursor-pointer items-center rounded-full border border-white/10 bg-black/20 shadow-inner transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-background disabled:cursor-not-allowed disabled:opacity-50 data-[state=checked]:bg-primary data-[state=unchecked]:bg-black/40 data-[state=checked]:shadow-glow-cyan hover:data-[state=unchecked]:bg-black/50",
      className,
    )}
    {...props}
    ref={ref}
  >
    ...
    <SwitchPrimitives.Thumb
      className={cn(
        "absolute left-0.5 pointer-events-none block h-6 w-6 rounded-full bg-white shadow-lg ring-0 transition-transform data-[state=checked]:translate-x-[28px] data-[state=unchecked]:translate-x-0 will-change-transform group-active:scale-95",
      )}
    />
  </SwitchPrimitives.Root>
));
