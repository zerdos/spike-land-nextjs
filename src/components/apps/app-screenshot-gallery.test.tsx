...
describe('AppScreenshotGallery', () => {
  ...
  describe('Empty State', () => {
    ...
    it('applies custom className to empty state', () => {
      const { container } = render(<AppScreenshotGallery screenshots={[]} className="custom-empty" />)
  ...
  describe('Thumbnail Grid Rendering', () => {
    ...
    it('renders with 2 columns when specified', () => {
      const { container } = render(<AppScreenshotGallery screenshots={mockScreenshots} columns={2} />)
    ...
    it('renders with 4 columns when specified', () => {
      ...
      const grid = container.querySelector('.grid-cols-1.sm\\:grid-cols-2.lg\\:grid-cols-3.xl\\:grid-cols-4')
      expect(grid).toBeInTheDocument()
    })

    it('applies custom className to gallery container', () => {
      const { container } = render(<AppScreenshotGallery screenshots={mockScreenshots} className="custom-gallery" />)
  ...
  describe('Image Properties', () => {
    it('thumbnail images have correct attributes', () => {
      ...
      const thumbnailImages = images.filter(img =>
        img.getAttribute('data-sizes') === '(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 33vw'
      )
    ...
    it('modal images have correct sizes attribute', async () => {
      ...
      await vi.waitFor(() => {
        ...
        const modalImage = images.find(img =>
          img.getAttribute('data-sizes') === '90vw'
        )
  ...
  describe('Edge Cases', () => {
    ...
    it('handles very long screenshot titles', () => {
      ...
      expect(titleElement).toHaveTextContent('This is a very long screenshot title that should be truncated properly')
    })
