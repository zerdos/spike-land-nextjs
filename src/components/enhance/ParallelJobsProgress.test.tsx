...
describe("ParallelJobsProgress Component", () => {
  describe("Rendering", () => {
    ...
    it("applies custom className", () => {
      ...
      expect(screen.getByTestId("parallel-jobs-progress")).toHaveClass(
        "custom-class",
      );
    });
  });
  ...
  describe("Progress Summary", () => {
    ...
    it("shows failed count when there are failures", () => {
      const jobs = [
        ...
        createJob("TIER_2K", "COMPLETED", {
          enhancedUrl: "http://example.com/2.jpg",
        }),
      ];
      render(<ParallelJobsProgress jobs={jobs} />);
      expect(screen.getByTestId("progress-summary")).toHaveTextContent(
  ...
  describe("Status Icons", () => {
    it("renders Clock icon for PENDING status", () => {
      const jobs = [createJob("TIER_1K", "PENDING")];
      render(<ParallelJobsProgress jobs={jobs} />);
      expect(screen.getByTestId("status-icon-pending-TIER_1K"))
        .toBeInTheDocument();
    });
    ...
    it("renders Check icon for COMPLETED status", () => {
      ...
      render(<ParallelJobsProgress jobs={jobs} />);
      expect(screen.getByTestId("status-icon-completed-TIER_1K"))
        .toBeInTheDocument();
    });
    ...
    it("renders X icon for FAILED status", () => {
      ...
      render(<ParallelJobsProgress jobs={jobs} />);
      expect(screen.getByTestId("status-icon-failed-TIER_1K"))
        .toBeInTheDocument();
    });
    ...
    it("applies tier-specific color to PROCESSING icon", () => {
      ...
      expect(screen.getByTestId("status-icon-processing-TIER_1K")).toHaveClass(
        "text-green-400",
      );
      expect(screen.getByTestId("status-icon-processing-TIER_2K")).toHaveClass(
        "text-cyan-400",
      );
      expect(screen.getByTestId("status-icon-processing-TIER_4K")).toHaveClass(
        "text-purple-400",
      );
    });
  });
  ...
  describe("Status Text", () => {
    it("shows 'Queued' text for PENDING status", () => {
      const jobs = [createJob("TIER_1K", "PENDING")];
      render(<ParallelJobsProgress jobs={jobs} />);
      expect(screen.getByTestId("pending-text-TIER_1K")).toHaveTextContent(
        "Queued",
      );
    });
    ...
    it("shows 'Processing...' text for PROCESSING status", () => {
      const jobs = [createJob("TIER_1K", "PROCESSING")];
      render(<ParallelJobsProgress jobs={jobs} />);
      expect(screen.getByTestId("processing-text-TIER_1K")).toHaveTextContent(
        "Processing...",
      );
    });
  });
  ...
  describe("Completed State", () => {
    ...
    it("does not show View button if onViewResult is not provided", () => {
      ...
      fireEvent.click(screen.getByTestId("toggle-view-button"));
      expect(screen.queryByTestId("view-result-TIER_1K")).not
        .toBeInTheDocument();
    });
  });
  ...
  describe("Failed State", () => {
    it("shows error message when job has failed", () => {
      const jobs = [
        createJob("TIER_1K", "FAILED", {
          error: "Enhancement failed due to timeout",
        }),
      ];
      render(<ParallelJobsProgress jobs={jobs} />);
    ...
    it("does not show retry button when onRetry is not provided", () => {
      const jobs = [createJob("TIER_1K", "FAILED", { error: "Error" })];
      render(<ParallelJobsProgress jobs={jobs} />);
      expect(screen.queryByTestId("retry-button-TIER_1K")).not
        .toBeInTheDocument();
    });
    ...
    it("applies destructive border style to failed job card", () => {
      const jobs = [createJob("TIER_1K", "FAILED", { error: "Error" })];
      render(<ParallelJobsProgress jobs={jobs} />);
      expect(screen.getByTestId("job-card-TIER_1K")).toHaveClass(
    ...
    it("does not show error message if error is undefined", () => {
      ...
      render(<ParallelJobsProgress jobs={jobs} />);
      expect(screen.queryByTestId("error-message-TIER_1K")).not
        .toBeInTheDocument();
    });
  });
  ...
  describe("Card Styling", () => {
    ...
    it("does not apply special border to pending or processing cards", () => {
      ...
      expect(screen.getByTestId("job-card-TIER_1K")).not.toHaveClass(
        "border-destructive/50",
      );
      expect(screen.getByTestId("job-card-TIER_1K")).not.toHaveClass(
        "border-green-500/30",
      );
      expect(screen.getByTestId("job-card-TIER_2K")).not.toHaveClass(
        "border-destructive/50",
      );
      expect(screen.getByTestId("job-card-TIER_2K")).not.toHaveClass(
        "border-green-500/30",
  ...
  describe("Expanded/Compact View Toggle", () => {
    it("starts in expanded view by default", () => {
      ...
      expect(screen.getByTestId("toggle-view-button")).toHaveTextContent(
        "Compact",
      );
      expect(screen.getByTestId("job-thumbnail-TIER_1K")).toBeInTheDocument();
    });
    ...
    it("toggles to compact view when button is clicked", () => {
      ...
      expect(screen.queryByTestId("job-thumbnail-TIER_1K")).not
        .toBeInTheDocument();
      expect(screen.getByTestId("view-result-TIER_1K")).toBeInTheDocument();
    });
  ...
  describe("Multiple Jobs Integration", () => {
    it("renders all three tier jobs correctly", () => {
      ...
      expect(screen.getByTestId("status-icon-completed-TIER_1K"))
        .toBeInTheDocument();
      expect(screen.getByTestId("status-icon-processing-TIER_2K"))
        .toBeInTheDocument();
      expect(screen.getByTestId("status-icon-failed-TIER_4K"))
        .toBeInTheDocument();
  ...
  describe("Edge Cases", () => {
    ...
    it("renders job card for all status types without callbacks", () => {
      const jobs = [
        createJob("TIER_1K", "PENDING"),
        createJob("TIER_2K", "PROCESSING"),
        createJob("TIER_4K", "COMPLETED", {
          enhancedUrl: "http://example.com/4k.jpg",
        }),
      ];
      render(<ParallelJobsProgress jobs={jobs} />);
