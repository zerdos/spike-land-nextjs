      ...
      <DialogContent className="max-w-5xl h-[90vh] p-0 overflow-hidden bg-black/95 border-white/10">
        <div className="relative h-full w-full flex flex-col md:flex-row">
          ...
          <div className="w-full md:w-80 flex-shrink-0 bg-zinc-900 p-6 border-l border-white/10 flex flex-col gap-6 overflow-y-auto">
            ...
            {image.tags.length > 0 && (
                <div className="space-y-2">
                    <h4 className="text-xs font-semibold uppercase tracking-wider text-zinc-500">Tags</h4>
            ...
            <div className="space-y-3 mt-auto pt-6 border-t border-white/10">
                <div className="flex justify-between text-sm">
                    ...
                    <span className="text-zinc-300 font-mono">{job?.tier?.replace("TIER_", "") || "Original"}</span>
                </div>
                {job?.enhancementType && (
                    <div className="flex justify-between text-sm">
                ...
                <div className="flex justify-between text-sm">
                    ...
                    <span className="text-zinc-300 font-mono">
                        {job?.enhancedWidth ? `${job.enhancedWidth}x${job.enhancedHeight}` : `${image.originalWidth}x${image.originalHeight}`}
                    </span>
                </div>
            </div>
