...
import { createStableUserId, authConfig } from './auth.config'
...
describe('authConfig', () => {
  ...
  describe('session callback', () => {
    it('should add user id from token.sub', () => {
      ...
      const result = authConfig.callbacks?.session?.({ session, token } as Parameters<NonNullable<typeof authConfig.callbacks.session>>[0])
  ...
  describe('jwt callback', () => {
    it('should use stable ID for users with email', () => {
      ...
      const result = authConfig.callbacks?.jwt?.({ token, user } as Parameters<NonNullable<typeof authConfig.callbacks.jwt>>[0])
    ...
    it('should preserve existing token.sub when no user', () => {
      ...
      const result = authConfig.callbacks?.jwt?.({ token } as Parameters<NonNullable<typeof authConfig.callbacks.jwt>>[0])
