...
__export(index_exports, {
  ...
  AddToCartRequestSchema: () => AddToCartRequestSchema,
  AlbumPrivacySchema: () => AlbumPrivacySchema,
  AspectRatioSchema: () => AspectRatioSchema,
  BatchEnhanceRequestSchema: () => BatchEnhanceRequestSchema,
  CheckEmailRequestSchema: () => CheckEmailRequestSchema,
  CreateAlbumRequestSchema: () => CreateAlbumRequestSchema,
  CreateOrderRequestSchema: () => CreateOrderRequestSchema,
  ENHANCEMENT_COSTS: () => ENHANCEMENT_COSTS,
  EmailSchema: () => EmailSchema,
  EnhanceImageRequestSchema: () => EnhanceImageRequestSchema,
  EnhancementTierSchema: () => EnhancementTierSchema,
  GenerateImageRequestSchema: () => GenerateImageRequestSchema,
  IMAGE_CONSTRAINTS: () => IMAGE_CONSTRAINTS,
  MCP_GENERATION_COSTS: () => MCP_GENERATION_COSTS,
  ModifyImageRequestSchema: () => ModifyImageRequestSchema,
  REFERRAL_CONFIG: () => REFERRAL_CONFIG,
  REVENUECAT_PRODUCTS: () => REVENUECAT_PRODUCTS,
  RedeemVoucherRequestSchema: () => RedeemVoucherRequestSchema,
  ...
  TOKEN_REGENERATION: () => TOKEN_REGENERATION,
  UpdateAlbumRequestSchema: () => UpdateAlbumRequestSchema,
  calculateOutputDimensions: () => calculateOutputDimensions,
  calculateRegeneratedTokens: () => calculateRegeneratedTokens,
  formatCurrency: () => formatCurrency,
  formatDuration: () => formatDuration,
  formatFileSize: () => formatFileSize,
  formatRelativeTime: () => formatRelativeTime,
  generateShareToken: () => generateShareToken,
  getEnhancementCost: () => getEnhancementCost,
  getTierDimensions: () => getTierDimensions,
  getTimeUntilNextRegen: () => getTimeUntilNextRegen,
...
var EnhancementTierSchema = import_zod.z.enum([
  "FREE",
  "TIER_1K",
  "TIER_2K",
  "TIER_4K"
]);
var EnhanceImageRequestSchema = import_zod.z.object({
  imageId: import_zod.z.string().min(1),
  tier: EnhancementTierSchema,
  prompt: import_zod.z.string().optional(),
  pipelineId: import_zod.z.string().optional()
});
var BatchEnhanceRequestSchema = import_zod.z.object({
  imageIds: import_zod.z.array(import_zod.z.string().min(1)).min(1).max(IMAGE_CONSTRAINTS.MAX_BATCH_SIZE),
  tier: EnhancementTierSchema
});
var AspectRatioSchema = import_zod.z.enum(SUPPORTED_ASPECT_RATIOS);
var GenerateImageRequestSchema = import_zod.z.object({
  prompt: import_zod.z.string().min(1).max(4e3),
  tier: EnhancementTierSchema.default("TIER_1K"),
  aspectRatio: AspectRatioSchema.default("1:1"),
  negativePrompt: import_zod.z.string().max(1e3).optional()
});
var ModifyImageRequestSchema = import_zod.z.object({
  prompt: import_zod.z.string().min(1).max(4e3),
  tier: EnhancementTierSchema.default("TIER_1K"),
  imageUrl: import_zod.z.string().url().optional(),
  imageBase64: import_zod.z.string().optional()
});
var AlbumPrivacySchema = import_zod.z.enum(["PRIVATE", "UNLISTED", "PUBLIC"]);
var CreateAlbumRequestSchema = import_zod.z.object({
...
var UpdateAlbumRequestSchema = import_zod.z.object({
  ...
  coverImageId: import_zod.z.string().optional()
});
var RedeemVoucherRequestSchema = import_zod.z.object({
  code: import_zod.z.string().min(4).max(20).transform((val) => val.toUpperCase().trim())
});
var AddToCartRequestSchema = import_zod.z.object({
  productId: import_zod.z.string().min(1),
  variantId: import_zod.z.string().optional(),
  imageId: import_zod.z.string().optional(),
  uploadedImageUrl: import_zod.z.string().url().optional(),
  quantity: import_zod.z.number().int().min(1).max(10).default(1),
  customText: import_zod.z.string().max(100).optional()
});
var ShippingAddressSchema = import_zod.z.object({
  name: import_zod.z.string().min(1).max(100),
  line1: import_zod.z.string().min(1).max(200),
  line2: import_zod.z.string().max(200).optional(),
  city: import_zod.z.string().min(1).max(100),
  state: import_zod.z.string().max(100).optional(),
  postalCode: import_zod.z.string().min(1).max(20),
  country: import_zod.z.string().length(2)
  // ISO 2-letter country code
});
var CreateOrderRequestSchema = import_zod.z.object({
...
var CheckEmailRequestSchema = import_zod.z.object({
  email: EmailSchema
});
var SignUpRequestSchema = import_zod.z.object({
  email: EmailSchema,
  password: import_zod.z.string().min(8).max(100),
  name: import_zod.z.string().min(1).max(100).optional(),
  referralCode: import_zod.z.string().max(20).optional()
});
