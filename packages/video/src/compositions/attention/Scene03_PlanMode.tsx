        ...
        <AbsoluteFill>
            <div style={{ padding: 80, display: 'flex', flexDirection: 'column', gap: 40, height: '100%' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <KineticText text="MODE SWITCH" fontSize={50} color={COLORS.cyan} type="reveal" />
                    <div style={{
                        borderRadius: 8,
                        color: 'white',
                        fontWeight: 'bold',
                ...
                <SplitLayout
                    left={
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                            <div style={{ fontSize: 40, fontWeight: 700, color: COLORS.textPrimary }}>CAN DO:</div>
                            <div style={{ color: COLORS.success, fontSize: 30 }}>✓ Read Local Files</div>
                            <div style={{ color: COLORS.success, fontSize: 30 }}>✓ Search Codebase</div>
                            <div style={{ color: COLORS.success, fontSize: 30 }}>✓ Architect Strategy</div>

                            <div style={{ fontSize: 40, fontWeight: 700, color: COLORS.textPrimary, marginTop: 20 }}>CANNOT DO:</div>
                            <div style={{ color: COLORS.error, fontSize: 30 }}>✗ Write Code</div>
                            <div style={{ color: COLORS.error, fontSize: 30 }}>✗ Delete Files</div>
                            <div style={{ color: COLORS.error, fontSize: 30 }}>✗ Execute Shell</div>
                        </div>
                    }
                    right={
                        <CodeBlock
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 40 }}>
                {['EXPLORE', 'PLAN', 'CODE'].map((step, i) => (
                    <Fragment key={step}>
                        ...
                        {i < 2 && <div style={{ fontSize: 50, color: COLORS.cyan }}>→</div>}
                    </Fragment>
                ))}
            </div>
            <QuoteBlock
      ...
      {frame >= 1872 && (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', gap: 80 }}>
            <div style={{
                ...
                <div style={{ fontSize: 30, color: COLORS.cyan, marginBottom: 20, fontFamily: 'monospace' }}>plan.md</div>
                <div style={{ fontSize: 40, fontWeight: 700, color: 'white' }}>TECHNICAL STRATEGY</div>
                <div style={{ marginTop: 20, color: COLORS.textSecondary, fontSize: 26 }}>• Reference src/lib/auth.ts:42</div>
                <div style={{ color: COLORS.textSecondary, fontSize: 26 }}>• Update checkPermission() logic</div>
