    ...
    <AbsoluteFill>
      ...
      {frame >= 300 && frame < 1200 && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 40, padding: 80, height: '100%' }}>
            <KineticText text="Tactic 1: Explore First" fontSize={76} color={COLORS.cyan} type="reveal" />

            <SplitLayout
                left={
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                        <div style={{ color: COLORS.error, fontSize: 30, fontWeight: 800 }}>THE "HELP" LOOP ❌</div>
                        <ChatBubble message="HELP!! TypeError: Cannot read 'map' of undefined. [Pasts 1000 lines of code]" />
                        <div style={{ fontSize: 22, color: COLORS.textMuted }}>Result: Confusion & Hallucinations</div>
                    </div>
                }
                right={
                   <div style={{ display: 'flex', flexDirection: 'column', gap: 15 }}>
                        <div style={{ color: COLORS.success, fontSize: 30, fontWeight: 800 }}>MISE EN PLACE ✅</div>
      ...
      {frame >= 1200 && frame < 2100 && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 40, padding: 80, height: '100%' }}>
            <KineticText text="Tactic 2: Reference, Don't Describe" fontSize={76} color={COLORS.cyan} type="reveal" />
      ...
      {frame >= 2100 && frame < 3000 && (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', gap: 60 }}>
            <KineticText text="Tactic 3: The Clean Room" fontSize={76} color={COLORS.cyan} type="reveal" />

            <div style={{ display: 'flex', alignItems: 'center', gap: 40 }}>
                <div style={{ textAlign: 'center' }}>
                    <div style={{ fontSize: 100 }}>🗑️</div>
                    <div style={{ color: COLORS.textMuted, fontSize: 26 }}>Old Chat History</div>
                </div>
                ...
                <div style={{ textAlign: 'center', transform: `scale(${spring({ frame: frame - 2200, fps, config: SPRING_CONFIGS.bouncy })})` }}>
                    ...
                    <div style={{ color: COLORS.success, fontWeight: 800, fontSize: 26 }}>FRESH SESSION</div>
                </div>
                <div style={{ fontSize: 76, color: COLORS.cyan }}>→</div>
                <div style={{ textAlign: 'center', transform: `scale(${spring({ frame: frame - 2300, fps, config: SPRING_CONFIGS.bouncy })})` }}>
                    <div style={{ fontSize: 100 }}>📋</div>
                    <div style={{ color: COLORS.cyan, fontSize: 26 }}>PASTE PLAN</div>
                </div>
            </div>
      ...
      {frame >= 3000 && (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', gap: 40, padding: 80 }}>
            <KineticText text="The 5-Layer Context Stack" fontSize={76} color={COLORS.cyan} type="reveal" />
            ...
            <div style={{ marginTop: 20, fontSize: 40, fontWeight: 800, color: 'white' }}>
