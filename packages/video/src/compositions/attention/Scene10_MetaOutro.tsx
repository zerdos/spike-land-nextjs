    ...
    <AbsoluteFill>
      ...
      {frame >= 300 && frame < 1000 && (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', gap: 60 }}>
            <KineticText text="16 Parallel Agents" fontSize={76} color={COLORS.cyan} type="reveal" />
      ...
      {frame >= 1000 && frame < 1500 && (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', gap: 40 }}>
            <div style={{ fontSize: 50, color: COLORS.textSecondary }}>The article about context engineering...</div>
            <div style={{ transform: `scale(${interpolate(frame, [1100, 1400], [1, 5], { extrapolateRight: 'clamp', extrapolateLeft: 'clamp' })})`, opacity: interpolate(frame, [1300, 1500], [1, 0], { extrapolateRight: 'clamp', extrapolateLeft: 'clamp' }) }}>
                <KineticText text="Was Context Engineered" fontSize={100} color={COLORS.cyan} type="reveal" />
            </div>
            {frame >= 1100 && (
                <div style={{ fontSize: 40, color: COLORS.fuchsia, fontWeight: 800 }}>RECURSIVE EXCELLENCE</div>
            )}
        </div>
      )}
      ...
      {frame >= 1500 && frame < 2000 && (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', gap: 60 }}>
            ...
            <div style={{ textAlign: 'center' }}>
                <KineticText text="Stop filling your pie with junk." fontSize={64} color="white" type="reveal" />
                <div style={{ marginTop: 20 }}>
                    <KineticText text="Check your context hygiene." fontSize={40} color={COLORS.cyan} type="slide" direction="bottom" delay={40} />
                </div>
            </div>
        </div>
      )}
      ...
      {frame >= 2000 && (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', gap: 40 }}>
            ...
            <div style={{ marginTop: 40, opacity: interpolate(frame, [2050, 2100], [0, 1], { extrapolateRight: 'clamp', extrapolateLeft: 'clamp' }) }}>
                <KineticText text="Thanks for watching" fontSize={50} color={COLORS.textSecondary} type="reveal" />
            </div>
            <div style={{ opacity: interpolate(frame, [2150, 2200, 2274 - 20, 2274], [0, 1, 1, 0], { extrapolateRight: 'clamp', extrapolateLeft: 'clamp' }) }}>
                <KineticText text="See you next time." fontSize={30} color={COLORS.textMuted} type="reveal" delay={20} />
            </div>
        </div>
      )}
    </AbsoluteFill>
